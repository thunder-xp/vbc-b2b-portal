...
export default async function FinalCustomerOrdersPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  return <main className="mx-auto max-w-5xl space-y-5 px-4 py-6 sm:py-8">
    ...
    {orders.length ? <ul className="divide-y divide-zinc-200 overflow-hidden rounded-xl border border-zinc-200 bg-white">{orders.map((order) => <li key={order.id}><Link className="grid min-h-24 gap-3 p-4 hover:bg-zinc-50 sm:grid-cols-[56px_minmax(0,1fr)_220px_auto] sm:items-center sm:gap-5" href={`/account/orders/${order.id}`}>
      <span className="relative hidden size-14 overflow-hidden rounded-lg bg-zinc-100 sm:block">{order.previewImageUrl ? <Image alt="" fill className="object-contain p-1" sizes="56px" src={order.previewImageUrl} /> : null}</span>
