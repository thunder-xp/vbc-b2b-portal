...
export default async function CustomerDocumentsPage({ searchParams }: { searchParams: Promise<{ orderId?: string }> }) {
  ...
  return <main className={cabinetPage}>
    <WorkspaceHeader title={ro ? "Documente" : "Документы"} description={ro ? "Documentele publicate sunt grupate după cumpărătură și produs." : "Опубликованные документы сгруппированы по покупке и товару."} />
    {groups.length ? <div className="space-y-4">
      {groups.map((group) => <section className={`overflow-hidden ${cabinetSurface}`} key={group.orderId}>
        ...
        <ul className="divide-y divide-zinc-100">
          {group.products.map((product) => <li className="p-4" id={`purchase-${product.lineId}`} key={product.lineId}>
            <Link className="inline-flex min-h-11 items-center font-semibold hover:text-emerald-700 focus-visible:outline-2 focus-visible:outline-emerald-700" href={`/account/equipment/${product.lineId}`}>{product.name}</Link>
            <p className="text-xs text-zinc-500">SKU {product.sku}</p>
            <ul className="mt-2 grid gap-2 sm:grid-cols-2">
              {product.documents.map((document) => <li key={document.id}>
                <a className="flex min-h-11 items-center gap-2 rounded-lg border border-zinc-200 px-3 text-sm font-semibold text-emerald-700 transition-colors hover:border-emerald-300 hover:bg-emerald-50 focus-visible:outline-2 focus-visible:outline-emerald-700 motion-reduce:transition-none" href={document.url} rel="noreferrer" target="_blank">
