...
import { customerDate, customerMoney, orderStatus, orderStatusTone, paymentStatus, paymentStatusTone, serviceStatusLabel, serviceStatusTone } from "@/src/modules/final-customer/presentation";
      ...
      {!hasActivity ? (
        ...
        <div className="space-y-8">
          ...
          <section className="space-y-3" aria-labelledby="customer-objects-heading">
            ...
            />
            {objectWorkspace.objects.length ? <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{objectWorkspace.objects.slice(0, 3).map((object) => <Link className={`group flex min-h-20 items-center gap-3 p-4 ${cabinetSurface} ${cabinetRow}`} href={`/account/objects/${object.id}`} key={object.id}><span className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-emerald-50 text-emerald-700"><Building2 aria-hidden className="size-5" /></span><span className="min-w-0 flex-1"><strong className="block truncate">{object.name}</strong><span className="block text-xs text-zinc-500">{customerObjectTypeLabels[locale][object.objectType]} · {ro ? `${object.productCount} produse` : `${object.productCount} товаров`}</span><span className={`mt-1 block text-xs ${object.openServiceCount ? "font-medium text-amber-700" : "text-zinc-500"}`}>{object.openServiceCount ? (ro ? `Service deschis: ${object.openServiceCount}` : `Открытый сервис: ${object.openServiceCount}`) : `${ro ? "Activitate" : "Активность"}: ${customerDate(object.lastActivityAt, locale)}`}</span></span><ArrowRight aria-hidden className="size-4 text-zinc-500" /></Link>)}</div>
