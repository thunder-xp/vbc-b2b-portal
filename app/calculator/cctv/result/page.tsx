      ...
      <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
        {!result || !input ? <FailureState locale={effectiveLocale} modifyHref={modifyHref} /> : <>
          ...
          <div className="mt-10 grid gap-10 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-start">
            ...
            <aside className="border border-zinc-200 bg-white p-5 lg:sticky lg:top-24"><p className="text-xs font-semibold uppercase text-emerald-700">{ru ? "Предварительный расчёт" : "Calcul preliminar"}</p><h2 className="mt-2 text-xl font-semibold">{ru ? "Ориентировочная стоимость системы" : "Costul estimativ al sistemului"}</h2><dl className="mt-5 space-y-3 text-sm"><TotalRow label={ru ? "Оборудование" : "Echipamente"} locale={input.locale} value={result.totals.equipment} currency={result.totals.currency} /><TotalRow label={ru ? "Материалы" : "Materiale"} locale={input.locale} value={result.totals.materials} currency={result.totals.currency} /><div className="flex justify-between gap-3"><dt>{ru ? "Монтаж" : "Instalare"}</dt><dd className="text-right text-zinc-500">{ru ? "Рассчитывается далее" : "Se calculează ulterior"}</dd></div></dl><div className="mt-5 border-t border-zinc-200 pt-5"><div className="flex items-end justify-between gap-4"><span className="font-semibold">{ru ? "Итого" : "Total"}</span><strong className="text-2xl tabular-nums">{result.totals.total !== null && result.totals.currency ? formatRetailPrice(result.totals.total, result.totals.currency, input.locale) : "—"}</strong></div></div><p className="mt-5 text-xs leading-5 text-zinc-500">{ru ? "Финальный объём монтажа и длина кабеля уточняются после проверки объекта." : "Volumul final al instalării și lungimea cablului se confirmă după verificarea obiectivului."}</p><div className="mt-6 grid gap-3"><Link className="inline-flex min-h-12 items-center justify-center bg-emerald-700 px-4 text-sm font-semibold text-white" href={`/catalog?lang=${input.locale}`}>{ru ? "Перейти к каталогу" : "Deschide catalogul"}</Link><Link className="inline-flex min-h-12 items-center justify-center gap-2 border border-zinc-300 px-4 text-sm font-semibold" href={modifyHref}><ArrowLeft className="size-4" />{ru ? "Изменить параметры" : "Modifică parametrii"}</Link></div></aside>
          </div>
        </>}
      </div>
    </main>
