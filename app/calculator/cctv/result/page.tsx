...
export default async function PublicCctvResultPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const recommendedTotal = result?.totals.total ?? null;
  const economyTotal = result?.economyTotals?.total ?? null;
  const economySaving = economySelected && recommendedTotal !== null && economyTotal !== null
    ? Math.max(0, recommendedTotal - economyTotal) : 0;
    ...
    <main className="min-h-[calc(100vh-4rem)] bg-zinc-50" lang={effectiveLocale}>
      <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
        {!result || !input ? <FailureState locale={effectiveLocale} modifyHref={modifyHref} /> : <>
          ...
          {result.unresolved.length ? <section className="mt-8 border-l-4 border-amber-500 bg-amber-50 p-5"><h2 className="font-semibold">{ru ? "Для части позиций требуется подтверждение специалиста" : "Unele poziții necesită confirmarea unui specialist"}</h2><ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-amber-950">{result.unresolved.map((item) => <li key={item}>{item}</li>)}</ul><p className="mt-3 text-sm text-amber-900">{ru ? "Расчёт можно продолжить. Менеджер Novotech согласует подходящую модель, наличие и окончательную стоимость." : "Puteți continua calculul. Managerul Novotech va confirma modelul potrivit, disponibilitatea și costul final."}</p></section> : null}

          <div className="mt-10 grid gap-10 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-start">
            <div className="space-y-10">{result.economyLines ? <section className="flex flex-wrap items-center justify-between gap-3 border border-zinc-200 bg-white p-4"><div><p className="font-semibold">{economySelected ? (ru ? "Эконом вариант" : "Varianta economică") : (ru ? "Рекомендуемая система" : "Sistem recomandat")}</p><p className="mt-1 text-sm text-zinc-600">{economySelected ? (ru ? "Более доступный вариант с сохранением выбранных параметров системы." : "O variantă mai accesibilă, păstrând parametrii selectați ai sistemului.") : (ru ? "Оптимальный вариант по технической совместимости и наличию." : "Varianta optimă după compatibilitate și disponibilitate.")}</p>{economySelected && economySaving > 0 && displayedTotals?.currency ? <p className="mt-2 text-sm font-semibold text-blue-700">{ru ? "Экономия" : "Economie"} {formatRetailPrice(economySaving, displayedTotals.currency, effectiveLocale)}</p> : null}</div><Link className="inline-flex min-h-11 items-center justify-center bg-blue-700 px-4 text-sm font-semibold text-white hover:bg-blue-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-700" href={`/calculator/cctv/result?${variantQuery.toString()}`}>{economySelected ? (ru ? "Вернуться к рекомендуемому" : "Înapoi la varianta recomandată") : (ru ? "Нужен вариант дешевле" : "Am nevoie de o variantă mai ieftină")}</Link></section> : null}{GROUPS.map((group) => {
            ...
            })}<section><h2 className="text-xl font-semibold">{ru ? "Почему выбрана эта конфигурация" : "De ce a fost aleasă această configurație"}</h2><ul className="mt-4 space-y-3 text-sm leading-6 text-zinc-700">{result.explanations.map((explanation) => <li className="flex gap-3" key={explanation}><CheckCircle2 className="mt-0.5 size-5 shrink-0 text-blue-700" />{explanation}</li>)}</ul></section></div>

            <aside className="border border-zinc-200 bg-white p-5 lg:sticky lg:top-24">
              <p className="text-xs font-semibold uppercase text-blue-700">{ru ? "Предварительный расчёт" : "Calcul preliminar"}</p>
              <h2 className="mt-2 text-xl font-semibold">{displayedTotals?.isPartial ? (ru ? "Стоимость известных позиций" : "Costul pozițiilor confirmate") : (ru ? "Ориентировочная стоимость системы" : "Costul estimativ al sistemului")}</h2>
              <dl className="mt-5 space-y-3 text-sm"><TotalRow label={ru ? "Оборудование" : "Echipamente"} locale={input.locale} value={displayedTotals?.equipment ?? null} currency={displayedTotals?.currency ?? null} /><TotalRow label={ru ? "Материалы" : "Materiale"} locale={input.locale} value={displayedTotals?.materials ?? null} currency={displayedTotals?.currency ?? null} />{installationRequested ? <TotalRow label={ru ? "Монтаж и настройка" : "Instalare și configurare"} locale={input.locale} value={displayedTotals?.installation ?? null} currency={displayedTotals?.currency ?? null} /> : null}</dl>
              <div className="mt-5 border-t border-zinc-200 pt-5"><div className="flex items-end justify-between gap-4"><span className="font-semibold">{displayedTotals?.isPartial ? (ru ? "Известная стоимость" : "Cost cunoscut") : (ru ? "Итого" : "Total")}</span><strong className="text-2xl tabular-nums">{displayedTotals?.total !== null && displayedTotals?.currency ? formatRetailPrice(displayedTotals.total, displayedTotals.currency, input.locale) : "—"}</strong></div></div>
              {displayedTotals?.isPartial ? <p className="mt-3 border-l-2 border-amber-400 pl-3 text-xs leading-5 text-amber-900">{ru ? "Стоимость неподтверждённых позиций уточнит менеджер. Этот расчёт не является полной суммой к оплате." : "Costul pozițiilor neconfirmate va fi precizat de manager. Acest calcul nu reprezintă suma integrală de plată."}</p> : null}
              <p className="mt-5 text-xs leading-5 text-zinc-500">{installationRequested ? (ru ? "Финальный объём монтажа и длина кабеля уточняются после проверки объекта." : "Volumul final al instalării și lungimea cablului se confirmă după verificarea obiectivului.") : (ru ? "Финальная длина кабеля уточняется после проверки объекта." : "Lungimea finală a cablului se confirmă după verificarea obiectivului.")}</p>
              <div className="mt-6 grid gap-3">{cartItems.length > 0 ? <PublicRetailAddSystemButton calculatorInput={{ ...input, selectedVariant: economySelected ? "economy" : "recommended", policyVersion: result.cameraSelection.policyVersion, provisionalRequirements: result.provisionalRequirements, ...(result.provisionalRequirements.length ? { paymentEligibility: "blocked_unresolved_requirements" } : {}) }} installationIntent={installationIntent} installationPricing={result.installationPricing} items={cartItems} locale={input.locale} offerEnabled={!result.provisionalRequirements.length} selectedVariant={economySelected ? "economy" : "recommended"} workScope={workScope} /> : null}<Link className="inline-flex min-h-12 items-center justify-center border border-zinc-300 px-4 text-sm font-semibold" href={`/catalog?lang=${input.locale}&view=all`}>{ru ? "Перейти к каталогу" : "Deschide catalogul"}</Link><Link className="inline-flex min-h-12 items-center justify-center border border-zinc-300 px-4 text-sm font-semibold" href={`/installation?lang=${input.locale}&system=cctv&object=${input.objectType === "horeca" ? "retail" : input.objectType}&from=/calculator/cctv/result#request`}>{ru ? "Обсудить с менеджером" : "Discută cu managerul"}</Link><Link className="inline-flex min-h-12 items-center justify-center gap-2 border border-zinc-300 px-4 text-sm font-semibold" href={modifyHref}><ArrowLeft className="size-4" />{ru ? "Изменить параметры" : "Modifică parametrii"}</Link></div>
            </aside>
          </div>
        </>}
      </div>
