...
export default async function AgentRewardsPage() {
  ...
  return <main className={cabinetPageWide}>
    ...
    <section><h2 className="mb-3 text-lg font-semibold">{copy.monthlyStatement}</h2>{view.items.length ? <div className="space-y-4">{groupByMonth(view.items, locale).map((group) => <div className={cabinetSurface} key={group.key}><h3 className="border-b border-zinc-100 px-4 py-3 text-sm font-semibold capitalize">{group.label}</h3><div className="divide-y divide-zinc-100">{group.items.map((item) => <Link className="flex min-h-16 items-center justify-between gap-4 p-4 hover:bg-zinc-50" href={`/agent/deals/${item.saleLinkId}`} key={item.saleLinkId}><span><strong className="block">{item.orderNumber}</strong><span className="text-xs text-zinc-500">{new Date(item.orderDate).toLocaleDateString(locale === "ro" ? "ro-MD" : "ru-MD")} · {agentRewardStateCopy[locale][item.state]}</span>{item.safeBlockedReason ? <span className="mt-1 block text-xs text-amber-800">{agentRewardBlockedReasonCopy[locale][item.safeBlockedReason]}</span> : null}{item.paidAt ? <span className="mt-1 block text-xs text-emerald-800">{locale === "ro" ? "Plătit la" : "Выплачено"}: {new Date(item.paidAt).toLocaleDateString(locale === "ro" ? "ro-MD" : "ru-MD")}</span> : null}</span><strong className="tabular-nums">{money(item.amount, item.currency, locale)}</strong></Link>)}</div></div>)}</div> : <p className="rounded-lg border border-dashed border-zinc-300 bg-white p-5 text-sm text-zinc-600">{copy.noRewards}</p>}</section>
  </main>;
}
