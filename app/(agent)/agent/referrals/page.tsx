...
import { agentDate, attributionStatus, eventLabel, referralNextAction } from "@/src/modules/agent-cabinet/operational-presentation";
...
import { CabinetEmptyState, OperationalStatusPair } from "@/src/modules/cabinet-experience/components";
...
export default async function AgentReferralsPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  return <main className="mx-auto max-w-6xl space-y-5 px-4 py-6 sm:py-8"><AgentPageHeader title={ro ? "Recomandări" : "Рекомендации"} actions={<Link className={primaryButton} href="/agent/qr#referral-link"><Plus size={18}/>{ro ? "Creează recomandare" : "Создать рекомендацию"}</Link>}/>
    {result.items.length ? <ul className="divide-y divide-zinc-200 overflow-hidden rounded-xl border border-zinc-200 bg-white">{result.items.map((item) => <li key={item.id}><Link className="grid min-h-28 gap-3 p-4 hover:bg-zinc-50 md:grid-cols-[minmax(0,1fr)_280px_240px] md:items-center" href={`/agent/referrals/${item.id}`}>
      ...
      <OperationalStatusPair primaryLabel={ro ? "Recomandare" : "Рекомендация"} primaryValue={<ReferralStatusBadge locale={locale} status={item.status}/>} secondaryLabel={ro ? "Atribuire" : "Закрепление"} secondaryValue={attributionStatus(item.attributionStatus, locale)} />
