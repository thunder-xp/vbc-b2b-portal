...
export default async function AdminAgentsPage() {
  ...
  return <main className="space-y-6">
    <header className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wide text-emerald-700">Коммерческая сеть</p><h1 className="mt-1 text-3xl font-semibold">Коммерческие агенты</h1><p className="mt-2 max-w-3xl text-sm text-zinc-600">Отдельные от партнёров профили, compliance и управляемая реферальная атрибуция.</p></div><Link className="inline-flex min-h-11 items-center rounded-md border border-zinc-300 px-4 text-sm font-semibold" href="/admin/agents/referrals">Рефералы</Link></header>
