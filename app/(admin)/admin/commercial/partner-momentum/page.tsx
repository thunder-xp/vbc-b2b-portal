...
export default async function PartnerMomentumAdminPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <Metric label="Рост" value={numeric(byStatus.growth)} />
      <Metric label="Стабильно" value={numeric(byStatus.stable)} />
      <Metric label="Снижение активности" value={numeric(byStatus.slowing)} />
      ...
      <Metric label="Недостаточно истории" value={numeric(byStatus.insufficient_history)} />
    ...
    <section aria-label="Партнёры по динамике">
      {data.items.length ? <div className="grid gap-3">
        {data.items.map((item) => <details className="group border border-zinc-200 bg-white" key={item.companyId}>
          <summary className="min-h-11 cursor-pointer list-none p-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500">
            <div className="grid gap-4 md:grid-cols-[minmax(0,1.4fr)_repeat(4,minmax(7rem,auto))] md:items-center">
              <div className="min-w-0"><h2 className="font-semibold text-zinc-950">{item.companyName}</h2><p className="mt-1 text-xs text-zinc-500">{item.managerName ?? "Менеджер не назначен"}</p><div className="mt-2 flex flex-wrap gap-1">{item.reasonCodes.slice(0,3).map((reason) => <span className="rounded bg-zinc-100 px-2 py-1 text-xs text-zinc-700" key={reason}>{reasonLabel(reason)}</span>)}</div><p className="mt-2 text-xs font-semibold text-emerald-700 group-open:hidden">Открыть диагностику</p><p className="mt-2 hidden text-xs font-semibold text-emerald-700 group-open:block">Скрыть диагностику</p></div>
          ...
          <div className="border-t border-zinc-200 p-4">
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              <Datum label="Обычный интервал заказов" value={item.normalOrderIntervalDays === null ? "Недостаточно данных" : `${formatNumber(item.normalOrderIntervalDays)} дн.`} />
              <Datum label="Текущий цикл" value={item.cycleOverrunRatio === null ? "Недостаточно данных" : `${formatNumber(item.cycleOverrunRatio)}× от обычного`} />
              <Datum label="Последний заказ" value={formatDate(item.lastOrderAt)} />
              <Datum label="Расчёт обновлён" value={formatDate(item.calculatedAt)} />
            </div>
            <div className="mt-5 grid gap-5 lg:grid-cols-2">
              <div><h3 className="text-sm font-semibold text-zinc-950">Основные причины</h3><ul className="mt-2 space-y-1 text-sm text-zinc-700">{item.reasonCodes.slice(0,5).map((reason) => <li key={reason}>• {reasonLabel(reason)}</li>)}</ul></div>
              <div><h3 className="text-sm font-semibold text-zinc-950">Рекомендуемые действия</h3><ul className="mt-2 space-y-1 text-sm text-zinc-700">{recommendations(item.reasonCodes).map((recommendation) => <li key={recommendation}>• {recommendation}</li>)}</ul></div>
            </div>
          </div>
        </details>)}
...
function recommendations(reasons: string[]): string[] {
  const values = [
    reasons.includes("active_cart_not_converted") ? "Уточнить, нужна ли помощь с оформлением корзины." : null,
    reasons.includes("template_not_used") ? "Напомнить об активном шаблоне закупки." : null,
    reasons.includes("price_opportunity_available") ? "Проверить актуальные возможности для закупки." : null,
    reasons.includes("campaign_available") ? "Проверить доступное специальное предложение." : null,
    reasons.includes("purchase_cycle_overdue") ? "Связаться с партнёром и уточнить ближайшую потребность." : null,
    reasons.includes("order_frequency_down") ? "Проверить потребность в повторной закупке." : null,
  ].filter((value): value is string => value !== null);
  return [...new Set(values)].slice(0, 3);
}
function formatDate(value: string | null): string { return value ? new Intl.DateTimeFormat("ru-RU", { dateStyle: "medium" }).format(new Date(value)) : "Нет данных"; }
function formatNumber(value: number): string { return new Intl.NumberFormat("ru-RU", { maximumFractionDigits: 2 }).format(value); }
