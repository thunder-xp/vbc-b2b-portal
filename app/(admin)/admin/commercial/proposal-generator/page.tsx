...
export default async function ProposalGeneratorAdminPage() {
  const context = await requireAdminPagePermission("admin.estimates.view");
  ...
  const technicalProfiles = profilesResult.success ? profilesResult.data.filter((profile) =>
    /^(cctv\.(nvr|storage|poe)\.|cctv\.(cable\.cat5e|mounting|ups)$)/.test(profile.profileKey)) : [];
  const metrics = [
  ...
  return <div className="space-y-6"><header><p className="text-sm font-semibold text-emerald-700">Пилот</p><h1 className="mt-1 text-2xl font-semibold">Генератор КП</h1><p className="mt-1 text-sm text-zinc-600">Агрегированные показатели использования. Тексты требований и коммерческие данные здесь не сохраняются.</p></header>
    ...
    {poolsResult.success && configurationsResult.success ? <AdminCctvCameraPools canManageServices={context.permissions.includes("admin.retail_marketplace.manage") && context.permissions.includes("admin.integrations.manage")} initialConfigurations={configurationsResult.data} initialRows={poolsResult.data} /> : <p className="border-l-4 border-amber-500 bg-amber-50 px-4 py-3 text-sm">{poolsResult.success ? configurationsResult.message : poolsResult.message}</p>}
    <details className="rounded-md border border-zinc-200 bg-white p-4"><summary className="cursor-pointer font-semibold">Технические соответствия</summary><p className="mt-2 text-sm text-zinc-600">Регистраторы, накопители, PoE и материалы. Камеры и услуги управляются выше.</p><div className="mt-4">{profilesResult.success ? <AdminProposalGeneratorProfiles initialProfiles={technicalProfiles} /> : <p className="border-l-4 border-amber-500 bg-amber-50 px-4 py-3 text-sm">{profilesResult.message}</p>}</div></details>
