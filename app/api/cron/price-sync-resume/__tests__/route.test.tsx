...
const mocks = vi.hoisted(() => ({ getState: vi.fn(), continueSync: vi.fn(), launch: vi.fn(), resumePendingProjection: vi.fn() }));
vi.mock("server-only", () => ({}));
vi.mock("@/src/lib/env", () => ({ getOneCEnv: () => ({}) }));
vi.mock("@/src/modules/integration/services", () => ({ createChunkedPriceSyncService: () => ({ getState: mocks.getState, continue: mocks.continueSync, resumePendingProjection: mocks.resumePendingProjection }) }));
...
describe("price sync cron resumer", () => {
  beforeEach(() => { vi.stubEnv("CRON_SECRET", "cron-secret"); mocks.getState.mockResolvedValue(state); mocks.continueSync.mockResolvedValue({ state, needsContinuation: true, pagesProcessedThisInvocation: 1 }); mocks.launch.mockResolvedValue({ status: 202 }); mocks.resumePendingProjection.mockResolvedValue({ status: "no_pending" }); });
