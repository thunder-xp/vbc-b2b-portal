import { AuthSmsDeliveryError } from "@/src/modules/final-customer-auth/auth-sms.service";
import {
  handleSupabaseSendSmsHook,
  SendSmsHookVerificationError,
} from "@/src/modules/final-customer-auth/send-sms-hook.service";
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    const result = await handleSupabaseSendSmsHook(rawPayload, request.headers);
    return new Response("{}", {
      status: 200,
      headers: {
