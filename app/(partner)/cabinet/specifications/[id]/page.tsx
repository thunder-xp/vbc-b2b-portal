...
export default async function ProjectSpecificationPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams?: Promise<{ productSearch?: string }> }) {
  ...
  const reservationEntry = result.data.status === ProjectSpecificationStatus.Approved ? await getReservationEntryAction(id) : null;

  if (!isDraft) {
    return <div className="space-y-8"><SpecificationDetail specification={result.data} />{reservationEntry?.success ? <ReservationEntry entry={reservationEntry.data} /> : null}</div>;
  }
