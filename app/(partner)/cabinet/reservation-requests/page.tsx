...
const GROUPS: Array<{ indicators: PlannedShipmentIndicator[]; title: string }> = [
  { indicators: ["overdue"], title: "Просрочено" },
  { indicators: ["today"], title: "Сегодня" },
  { indicators: ["soon"], title: "Ближайшие три дня" },
  { indicators: ["scheduled"], title: "Позже" },
];
  ...
  return <div className="mx-auto max-w-6xl space-y-6">
    ...
    {!result.success ? <p className="border-l-4 border-red-500 bg-red-50 px-4 py-3 text-sm text-red-800">Не удалось загрузить планируемые отгрузки.</p> : result.data.shipments.length ? <>
      <div className="space-y-4">
        {GROUPS.map((group) => {
          const shipments = result.data.shipments.filter((shipment) => group.indicators.includes(shipment.dateIndicator));
          if (!shipments.length) return null;
          return <section className="overflow-hidden rounded-md border border-zinc-200 bg-white" key={group.title}>
          <h2 className="border-b border-zinc-200 bg-zinc-50 px-4 py-3 text-sm font-semibold text-zinc-950">{group.title} · {shipments.length}</h2>
        ...
        <ul className="divide-y divide-zinc-200">{shipments.map((shipment) => <li className="grid gap-3 px-4 py-4 lg:grid-cols-[minmax(11rem,1fr)_9rem_9rem_7rem_8rem_11rem] lg:items-center" key={shipment.id}>
...
function RequestCell({ shipment }: { shipment: PlannedShipmentDto }) {
  ...
  return <div className="space-y-1 text-sm"><p className="font-semibold">{request.statusLabel}</p><p className="text-xs text-zinc-500">Текущая дата: {formatDate(shipment.deliveryDate!)}</p><p className="text-xs text-zinc-500">Запрошенная дата: {formatDate(request.requestedDate)}</p>{request.reviewComment ? <p className="text-xs text-zinc-600">Комментарий Novotech: {request.reviewComment}</p> : null}{request.status === "pending" ? <p className="text-xs text-zinc-500">Текущая дата действует до решения Novotech.</p> : null}{request.awaitingOneC && <p className="text-xs text-amber-700">Запрос одобрен. Ожидается обновление даты в 1С.</p>}{request.status === "pending" && <CancelOrderDateChangeButton requestId={request.id} />}</div>;
}
