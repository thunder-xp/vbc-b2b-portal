...
export default async function OpportunitiesPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <div className="space-y-6">
    ...
    {result.data.items.length ? <div className="grid gap-4 xl:grid-cols-2">{result.data.items.map((opportunity) => <OpportunityCard canAddToOrder={contextResult.success && contextResult.data.capabilities.productCard.canAddToOrder} canAddToSpecification={contextResult.success && contextResult.data.capabilities.productCard.canAddToSpecification} canManagePurchasingLists={contextResult.success && contextResult.data.capabilities.productCard.canManagePurchasingLists} key={opportunity.id} opportunity={opportunity} />)}</div> : <section className="border border-dashed border-zinc-300 bg-white px-6 py-12 text-center"><h2 className="font-semibold text-zinc-950">Сейчас новых возможностей нет</h2><p className="mt-1 text-sm text-zinc-600">Мы покажем здесь релевантные изменения по вашим закупкам, шаблонам и спискам.</p><Link className="mt-4 inline-flex min-h-11 items-center rounded-md bg-emerald-700 px-4 text-sm font-semibold text-white" href="/cabinet/catalog">Открыть каталог</Link></section>}
    <NumberedPagination ariaLabel="Страницы возможностей" currentPage={result.data.page} hrefForPage={(targetPage) => pageHref(filter, targetPage)} totalPages={result.data.totalPages} />
  </div>;
}
