...
import { NEW_ROLLING_PERIODS, RollingPeriodSelector, type NewRollingPeriod } from "@/src/modules/commerce-period";
...
export async function CatalogResults({
  ...
  return <div className="space-y-6">
    ...
    {resultsTitle || merchandisingLabel === "TOP" || merchandisingLabel === "NEW" ? <CatalogResultsHeader action={merchandisingLabel === "TOP" || merchandisingLabel === "NEW" ? <RollingPeriodSelector activePeriod={period} hrefForPeriod={(target) => buildCatalogHref({ attributeFilters, availability, brandId, categoryId, categoryIds, categorySet, collection, explicitAll, merchandisingLabel, page: 1, period: target, search, sort })} locale={locale} periods={merchandisingLabel === "NEW" ? NEW_ROLLING_PERIODS : undefined} /> : undefined} title={resultsTitle ?? (merchandisingLabel === "NEW" ? merchandisingLabelName("NEW", copy) : copy.popular)} /> : null}
