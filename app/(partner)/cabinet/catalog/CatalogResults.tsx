...
import { RollingPeriodSelector, type NewRollingPeriod, type RollingPeriodState } from "@/src/modules/commerce-period";
...
export async function CatalogResults({
  ...
  return <div className="space-y-6">
    ...
    {resultsTitle || merchandisingLabel === "TOP" || merchandisingLabel === "NEW" || merchandisingLabel === "HOT" ? <CatalogResultsHeader action={merchandisingLabel === "TOP" || merchandisingLabel === "NEW" || merchandisingLabel === "HOT" ? <RollingPeriodSelector activePeriod={periodState ?? null} hrefForPeriod={(target) => buildCatalogHref({ attributeFilters, availability, brandId, categoryId, categoryIds, categorySet, collection, explicitAll, merchandisingLabel, page: 1, period: target, search, sort })} locale={locale} /> : undefined} title={resultsTitle ?? merchandisingLabelName(merchandisingLabel!, copy)} /> : null}
...
function FilterChip({ href, label }: { href: string; label: string }) { return <Link className="max-w-52 truncate whitespace-nowrap rounded-full border border-zinc-300 bg-white px-2.5 py-1 text-xs text-zinc-700 hover:border-emerald-500" href={href} prefetch={false} title={label}>{label} ×</Link>; }
