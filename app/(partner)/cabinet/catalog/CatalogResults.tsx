...
export async function CatalogResults({
  ...
  return <div className="space-y-6">
    ...
    {resultsTitle || merchandisingLabel === "TOP" || merchandisingLabel === "NEW" ? <CatalogResultsHeader action={merchandisingLabel === "TOP" || merchandisingLabel === "NEW" ? <RollingPeriodSelector activePeriod={periodState ?? null} hrefForPeriod={(target) => buildCatalogHref({ attributeFilters, availability, brandId, categoryId, categoryIds, categorySet, collection, explicitAll, merchandisingLabel, page: 1, period: target, search, sort })} locale={locale} /> : undefined} title={resultsTitle ?? (merchandisingLabel === "NEW" ? merchandisingLabelName("NEW", copy) : copy.popular)} /> : null}
