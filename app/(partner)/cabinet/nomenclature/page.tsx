...
import { externalItemTypeLabel, workspaceCopy } from "@/src/modules/partner-locale";
...
export default async function NomenclaturePage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const shared = query.scope === "shared" && itemType && (query.search?.trim().length ?? 0) >= 2 ? await searchExternalNomenclatureAction({ query: query.search ?? "", itemType, scope: "shared" }) : null;
  ...
  return (
    <div className="min-w-0 space-y-5">
      ...
      <form className="grid min-w-0 gap-3 border-b border-zinc-200 pb-5 sm:grid-cols-[minmax(0,1fr)_14rem_auto]">
        ...
        <label><span className="sr-only">{copy.nomenclatureType}</span><select className="min-h-11 w-full rounded-md border border-zinc-300 bg-white px-3 text-sm" defaultValue={itemType ?? ""} name="type"><option value="">{copy.allTypes}</option>{(["equipment", "material", "service"] as const).map((type) => <option key={type} value={type}>{externalItemTypeLabel(locale, type)}</option>)}</select></label>
        <button className="min-h-11 rounded-md bg-zinc-900 px-4 text-sm font-semibold text-white" type="submit">{copy.find}</button>
      </form>
      <div className="flex justify-end"><a className="min-h-11 text-sm font-semibold text-emerald-700 underline" href={scopeHref(query)}>{query.scope === "shared" ? copy.searchOwn : copy.searchShared}</a></div>
      {shared ? shared.success ? <SharedNomenclatureResults records={shared.data}/> : <p className="border-l-4 border-red-500 bg-red-50 px-4 py-3 text-sm text-red-800">{copy.nomenclatureError}</p> : !result?.success ? <p className="border-l-4 border-red-500 bg-red-50 px-4 py-3 text-sm text-red-800">{copy.nomenclatureError}</p> : <><PartnerNomenclatureWorkspace records={result.data.records} /><NumberedPagination ariaLabel={copy.nomenclaturePages} currentPage={result.data.page} hrefForPage={(page) => pageHref(query, page)} locale={locale} totalPages={result.data.totalPages} /></>}
    </div>
  );
}
