...
import { listPartnerNomenclatureAction } from "@/src/modules/estimates/actions";
...
export default async function NomenclaturePage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const result = await listPartnerNomenclatureAction({ search: query.search, itemType, page: Number.parseInt(query.page ?? "1", 10) || 1 });
  ...
  return <div className="min-w-0 space-y-5">
    ...
    <form className="grid min-w-0 gap-3 border-b border-zinc-200 pb-5 sm:grid-cols-[minmax(0,1fr)_14rem_auto]">
      <label className="relative min-w-0"><Search aria-hidden="true" className="absolute left-3 top-3.5 size-4 text-zinc-400" /><span className="sr-only">Поиск номенклатуры</span><input className="min-h-11 w-full min-w-0 rounded-md border border-zinc-300 pl-9 pr-3 text-sm" defaultValue={query.search} name="search" placeholder="Название, производитель или модель" /></label>
