...
export default async function PartnerSearchPage({ searchParams }: { searchParams: Promise<{ q?: string | string[] }> }) {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-5">
      ...
      <form action="/cabinet/search" className="flex gap-2" role="search">
        ...
        <button className="min-h-11 rounded-md bg-zinc-950 px-5 text-sm font-semibold text-white" type="submit">{copy.find}</button>
      </form>
      {query.length < 2 ? <p className="border border-zinc-200 bg-white p-5 text-sm text-zinc-600">{copy.searchMin}</p> : result?.success ? <PartnerSearchResults groups={result.data} query={query} /> : <p className="border border-red-200 bg-red-50 p-5 text-sm text-red-800">{copy.searchUnavailable}</p>}
    </div>
  );
}
