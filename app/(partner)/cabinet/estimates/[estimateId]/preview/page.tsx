...
export default async function EstimateProposalPreviewPage({ params }: { params: Promise<{ estimateId: string }> }) {
  ...
  return <div className="-mx-4 -my-6 min-h-screen bg-zinc-100 lg:-mx-8 lg:-my-8"><ProposalControls automaticSenderDisplayName={result.data.proposal.branding.companyName} estimateId={result.data.estimateId} initialSettings={result.data.proposal.settings} revision={result.data.estimateRevision} selectedTemplateId={result.data.selectedTemplateId} templates={result.data.templates} /><main className="overflow-x-auto p-3 sm:p-8"><ProposalDocument proposal={result.data.proposal} /></main></div>;
}
