...
import { VersionProposalControls } from "@/src/modules/estimates/components/VersionProposalControls";
...
export default async function EstimateVersionPreviewPage({ params }: { params: Promise<{ estimateId: string; versionId: string }> }) {
  ...
  return <div className="-mx-4 -my-6 min-h-screen bg-zinc-100 lg:-mx-8 lg:-my-8"><BehaviorViewEvent dedupeKey={`proposal-preview:${versionId}`} eventName="proposal_previewed" route="/cabinet/estimates/version/preview" sourceSurface="proposal_preview" /><VersionProposalControls estimateId={estimateId} versionId={versionId} versionNumber={result.data.versionNumber} /><main className="overflow-x-auto p-3 sm:p-8"><ProposalDocument proposal={result.data.proposal} /></main></div>;
}
