...
export default async function NewEstimatePage({ searchParams }: { searchParams: Promise<{ productId?: string }> }) {
  const { productId } = await searchParams;
  const initialProductId = productId && /^[0-9a-f]{8}-[0-9a-f-]{27,}$/i.test(productId) ? productId : null;
  ...
  return (
    <div className="mx-auto w-full max-w-5xl space-y-5">
      ...
      {!currencies.success ? <p className="border-l-4 border-red-500 bg-red-50 px-4 py-3 text-sm text-red-800">{currencies.message}</p> : <section className="border-y border-zinc-200 bg-white px-4 py-5 sm:px-6"><EstimateCreateForm currencies={currencies.data} initialProductId={initialProductId} /></section>}
    </div>
  );
}
