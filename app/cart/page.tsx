...
function CartContent({ cart, checkoutAccess, locale, offer }: { cart: PublicRetailCartDto; checkoutAccess: boolean; locale: PublicRetailLocale; offer: PublicRetailCommercialOfferDto | null }) {
  ...
  return <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-start">
    ...
    <aside className="border border-zinc-200 bg-white p-5 lg:sticky lg:top-24">
      ...
      {checkoutAvailable ? <Link className="mt-6 inline-flex min-h-12 w-full items-center justify-center bg-blue-700 px-4 text-sm font-semibold text-white hover:bg-blue-800" href={`/checkout?lang=${locale}`}>{ru ? "Оформить заказ" : "Finalizează comanda"}</Link> : null}
