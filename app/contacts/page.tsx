      ...
      <section aria-labelledby="stores-heading" className="py-8">
        ...
        <div className="mt-5 grid border-l border-t border-zinc-200 md:grid-cols-2">
          {publicCompanyContent.stores.map((store) => <article className="border-b border-r border-zinc-200 p-5 sm:p-6" key={store.mapsHref}>
            ...
            <a className="mt-4 flex min-h-11 w-fit items-start gap-3 rounded-sm text-sm font-semibold text-blue-800 underline decoration-blue-300 underline-offset-4 hover:text-blue-950" href={store.mapsHref} rel="noopener noreferrer" target="_blank"><MapPin aria-hidden="true" className="mt-0.5 size-4 shrink-0" />{store.address[locale]}</a>
            <a className="mt-3 flex min-h-11 w-fit items-center gap-3 text-sm font-semibold text-blue-800 hover:text-blue-950" href={store.phone.href}><Phone aria-hidden="true" className="size-4" />{store.phone.display}</a>
            <p className="mt-3 flex items-start gap-3 text-sm leading-6 text-zinc-600"><Clock3 aria-hidden="true" className="mt-1 size-4 shrink-0 text-blue-700" /><span>{publicCompanyContent.hours.weekdays[locale]}<br />{publicCompanyContent.hours.saturday[locale]}</span></p>
          </article>)}
        </div>
      </section>
