...
import { ONE_C_PRICE_TYPE_FIELDS, ONE_C_PRICE_TYPE_VAT_INCLUDED_FIELD, ONE_C_RESOURCES } from "./one-c-odata-identifiers";

const PRICE_RESOURCE = "InformationRegister_\u0426\u0435\u043d\u044b\u041d\u043e\u043c\u0435\u043d\u043a\u043b\u0430\u0442\u0443\u0440\u044b";
const PRICE_TYPE_RESOURCE = ONE_C_RESOURCES.priceTypes;
...
const PRICE_TYPE_SELECT = ONE_C_PRICE_TYPE_FIELDS.join(",");
...
export type PriceTypeStageRow = { externalRef: string; externalCode: string; name: string; currencyRef: string | null; sourceVersion: string | null; vatIncluded: boolean | null; isActive: boolean };
...
export class OneCPriceChunkProvider implements PriceChunkProvider {
  ...
  fetchPriceTypes(skip: number, limit: number) { return this.page(PRICE_TYPE_RESOURCE, PRICE_TYPE_SELECT, "Ref_Key asc", skip, limit, mapPriceType, catalogStableKey); }
...
function mapPriceType(value: unknown): PriceTypeStageRow | null { if (!isRecord(value)) return null; const externalRef = parseRequiredOneCGuid(value.Ref_Key); if (!externalRef) return null; return { externalRef, externalCode: text(value.Code), name: text(value.Description), currencyRef: parseRequiredOneCGuid(value["\u0412\u0430\u043b\u044e\u0442\u0430\u0426\u0435\u043d\u044b_Key"]), sourceVersion: nullableText(value.DataVersion), vatIncluded: typeof value[ONE_C_PRICE_TYPE_VAT_INCLUDED_FIELD] === "boolean" ? value[ONE_C_PRICE_TYPE_VAT_INCLUDED_FIELD] : null, isActive: value.DeletionMark !== true }; }
