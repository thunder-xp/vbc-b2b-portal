...
describe("1C OData partner provider", () => {
  ...
  it("uses Basic Auth without exposing credentials in failures", async () => {
    ...
    await expect(provider().partners.searchPartners({ query: "Partner" })).rejects.toBeInstanceOf(IntegrationProviderUnavailableError);
  ...
  it("falls back to contract price type and excludes inactive contracts", async () => {
    vi.stubGlobal("fetch", sequence(collection([
      ...
      { ...contractRow(), Ref_Key: "deleted", DeletionMark: true },
      { ...contractRow(), Ref_Key: "inactive", Недействителен: true },
  ...
  it("never falls back to mock data in real mode", async () => {
    ...
    await expect(provider({ useMockPartners: false }).partners.searchPartners({ query: "Novotech Demo" })).rejects.toBeInstanceOf(IntegrationProviderUnavailableError);
  });
});
