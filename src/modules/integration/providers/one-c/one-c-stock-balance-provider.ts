...
const BALANCE_SELECT = ["\u041e\u0440\u0433\u0430\u043d\u0438\u0437\u0430\u0446\u0438\u044f_Key", "\u041d\u043e\u043c\u0435\u043d\u043a\u043b\u0430\u0442\u0443\u0440\u0430_Key", "\u0425\u0430\u0440\u0430\u043a\u0442\u0435\u0440\u0438\u0441\u0442\u0438\u043a\u0430_Key", "\u041f\u0430\u0440\u0442\u0438\u044f_Key", "\u0421\u0442\u0440\u0443\u043a\u0442\u0443\u0440\u043d\u0430\u044f\u0415\u0434\u0438\u043d\u0438\u0446\u0430_Key", "\u042f\u0447\u0435\u0439\u043a\u0430_Key", "\u041a\u043e\u043b\u0438\u0447\u0435\u0441\u0442\u0432\u043eBalance", "\u041a\u043e\u043b\u0438\u0447\u0435\u0441\u0442\u0432\u043e\u0418\u043d\u0442Balance"].join(",");
...
export class OneCStockBalanceProvider implements StockBalanceProvider {
  ...
  async fetchBalances(kind:StockBalanceKind,snapshotTime:string,skip:number,limit:number) { const resource=`${resourceFor(kind)}/Balance(Condition='',Dimensions='',Period=datetime'${formatOneCBusinessDateTime(snapshotTime)}')`; const params:Record<string,string>={"$top":String(limit),"$skip":String(skip)};if(kind==="physical")params["$select"]=BALANCE_SELECT;const payload=await this.collection(resource,params); return {rowCount:payload.length,items:aggregateStockRows(payload.flatMap(mapBalance))}; }
...
function mapBalance(value:unknown):StockStageRow[] { if(!record(value)) return []; const externalProductRef=parseRequiredOneCGuid(value["\u041d\u043e\u043c\u0435\u043d\u043a\u043b\u0430\u0442\u0443\u0440\u0430_Key"]); const externalWarehouseRef=parseRequiredOneCGuid(value["\u0421\u0442\u0440\u0443\u043a\u0442\u0443\u0440\u043d\u0430\u044f\u0415\u0434\u0438\u043d\u0438\u0446\u0430_Key"]); const externalCharacteristicRef=parseOneCGuid(value["\u0425\u0430\u0440\u0430\u043a\u0442\u0435\u0440\u0438\u0441\u0442\u0438\u043a\u0430_Key"]); const quantity=normalizeQuantity(value["\u041a\u043e\u043b\u0438\u0447\u0435\u0441\u0442\u0432\u043eBalance"]); return externalProductRef&&externalWarehouseRef&&externalCharacteristicRef&&quantity!==null?[{externalProductRef,externalWarehouseRef,externalCharacteristicRef,quantity}]:[]; }
export function aggregateStockRows(rows:StockStageRow[]):StockStageRow[] { const totals=new Map<string,{row:StockStageRow;milliunits:number}>(); for(const row of rows){const key=`${row.externalProductRef}:${row.externalWarehouseRef}:${row.externalCharacteristicRef}`;const milliunits=Math.round(row.quantity*1000);const current=totals.get(key);totals.set(key,{row:current?.row??row,milliunits:(current?.milliunits??0)+milliunits});} return [...totals.entries()].sort(([a],[b])=>a.localeCompare(b)).map(([,total])=>({...total.row,quantity:total.milliunits/1000})); }
function resourceFor(kind:StockBalanceKind){return kind==="physical"?PHYSICAL:kind==="reserved"?RESERVED:INCOMING;}
export function formatOneCBusinessDateTime(value:string){
  const date=new Date(value);
  if(!Number.isFinite(date.getTime()))throw new IntegrationValidationError("1C stock snapshot time is invalid.");
  const parts=new Intl.DateTimeFormat("en-CA",{timeZone:"Europe/Chisinau",year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",second:"2-digit",hourCycle:"h23"}).formatToParts(date);
  const part=(type:Intl.DateTimeFormatPartTypes)=>parts.find(item=>item.type===type)?.value;
  return `${part("year")}-${part("month")}-${part("day")}T${part("hour")}:${part("minute")}:${part("second")}`;
}
function record(value:unknown):value is Record<string,unknown>{return typeof value==="object"&&value!==null;}
function text(value:unknown){return typeof value==="string"?value.trim():"";}
function normalizeQuantity(value:unknown){if(typeof value!=="number"||!Number.isFinite(value))return null;const normalized=Math.round(value*1000)/1000;return Math.abs(normalized)<=99_999_999_999.999?normalized:null;}
