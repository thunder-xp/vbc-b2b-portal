...
export function CatalogSyncPanel() {
  ...
  return <div className="space-y-6">
    ...
    <SyncSection title="Partner prices" description="Обновляет цены из 1С для доступных типов цен.">
      ...
      <Report rows={priceState ? [["Status", priceStatus(priceState)], ["Current stage", priceState.currentStage ?? "-"], ["Started", priceState.startedAt ?? "Never"], ["Last successful run", priceState.lastSuccessfulSyncAt ?? "Never"], ["Pages processed", priceState.pagesProcessed], ["Rows scanned", priceState.rowsScanned], ["Rows staged", priceState.rowsStaged], ["Latest prices resolved", priceState.latestPricesResolved], ["Prices published", priceState.pricesPublished], ["Prices deactivated", priceState.pricesDeactivated], ["Unmatched products", priceState.unmatchedProducts], ["Unknown price types", priceState.unknownPriceTypes], ["Scan complete", String(priceState.scanComplete)], ["Failed stage", priceState.failedStage ?? "None"], ["Safe error", priceState.safeError ?? priceState.errorCategory ?? "None"], ["Last update", priceState.updatedAt]] : []} />
    </SyncSection>
