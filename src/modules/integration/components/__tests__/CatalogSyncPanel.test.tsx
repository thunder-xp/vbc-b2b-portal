...
const mocks = vi.hoisted(() => ({ runDaily: vi.fn(), getState: vi.fn(), prices: vi.fn(), stock: vi.fn() }));
vi.mock("../../actions", () => ({ runDailyCatalogSyncAction: mocks.runDaily, getDailyCatalogSyncStateAction: mocks.getState, syncPricesFromOneCAction: mocks.prices, syncStockFromOneCAction: mocks.stock }));
...
describe("CatalogSyncPanel", () => {
  beforeEach(() => { vi.clearAllMocks(); mocks.getState.mockResolvedValue({ success: true, data: state }); mocks.runDaily.mockResolvedValue({ success: true, message: "done", data: state }); mocks.prices.mockResolvedValue({ success: false }); mocks.stock.mockResolvedValue({ success: false }); });
