...
export class ChunkedStockSyncService {
  ...
  async continue(syncId:string){
    ...
    try{
      while(pages<MAX_PAGES&&this.now()-started<BUDGET){
        ...
        if(stage==="supplier_arrival_balance"){
          const page=await this.supplierProvider.fetchSupplierBalances(state.snapshotTime!,state.nextSkip,state.pageSize);
          await this.store.stageSupplierBalances(syncId,Math.floor(state.nextSkip/state.pageSize),page.items); pages++;
          const complete=page.rowCount<state.pageSize;
          await this.store.checkpoint(syncId,{stage:complete?"supplier_order_documents":stage,nextSkip:complete?0:state.nextSkip+state.pageSize,received:page.received,kind:"supplier_balance",supplierStats:{groups:page.groups,positive:page.items.length,excluded:page.excluded}});
          continue;
        }
    ...
    }catch(error){const current=await this.store.getState();await this.store.fail(syncId,current.currentStage??"physical_scan",current.pagesProcessed+1,error);return{state:await this.store.getState(),pages};}
  }
}
...
export class SupabaseStockSyncStore implements StockSyncStore {
  ...
  async stageBalances(id:string,kind:StockBalanceKind,sourcePage:number,rows:StockStageRow[]){const{data,error}=await createAdminClient().rpc("stage_stock_balance_rows",{p_sync_id:id,p_kind:kind,p_source_page:sourcePage,p_rows:rows.map(r=>({external_product_ref:r.externalProductRef,external_warehouse_ref:r.externalWarehouseRef,external_characteristic_ref:r.externalCharacteristicRef,quantity:r.quantity}))});if(error)throw dbError(error);return Number(data??0);}
  ...
  private async clear(id:string|null){if(!id)return;const client=createAdminClient();await Promise.all(["stock_balance_sync_stage","stock_warehouse_sync_stage","supplier_arrival_balance_stage","supplier_order_document_stage"].map(table=>client.from(table).delete().eq("sync_id",id)));}
}
