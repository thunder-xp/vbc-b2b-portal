...
export type StockSyncState = { status:StockSyncStatus; activeSyncId:string|null; lastFailedSyncId:string|null; snapshotTime:string|null; currentStage:StockSyncStage|null; nextSkip:number; pageSize:number; pagesProcessed:number; physicalRows:number; reservedRows:number; incomingRows:number; warehousesLoaded:number; supplierBalanceRows?:number; supplierBalanceGroups?:number; supplierPositiveGroups?:number; supplierNonpositiveExcluded?:number; supplierOrdersRequested?:number; productsMatched:number; productsUnmatched:number; rowsPublished:number; rowsDeactivated:number; stockStagedRows?:number; arrivalsStagedRows?:number; stockDeltaUnchanged?:number; stockDeltaInserted?:number; stockDeltaUpdated?:number; stockDeltaRemoved?:number; arrivalsDeltaUnchanged?:number; arrivalsDeltaInserted?:number; arrivalsDeltaUpdated?:number; arrivalsDeltaRemoved?:number; publicationDbMs?:number|null; publicationApplicationMs?:number|null; publicationTimeoutBudgetMs?:number; publicationHeadroomPercent?:number|null; publicationLockWaitMs?:number|null; publicationTriggerRows?:number; scanComplete:boolean; startedAt:string|null; lastSuccessfulSyncAt:string|null; errorCategory:string|null; failedStage:string|null; safeError:string|null; failedPage:number|null; updatedAt:string };
...
export class SupabaseStockSyncStore implements StockSyncStore {
  ...
  async publish(id:string){const client=createAdminClient();const{error:prepareError}=await client.rpc("prepare_exact_stock_publication",{p_sync_id:id});if(prepareError)throw dbError(prepareError);const started=performance.now();const{error}=await client.rpc("publish_exact_stock_snapshot",{p_sync_id:id});const applicationMs=Math.max(0,Math.round(performance.now()-started));if(error)throw dbError(error,{applicationMs});const{error:metricError}=await client.from("stock_sync_state").update({publication_application_ms:applicationMs}).eq("id","exact_stock").eq("last_completed_sync_id",id);if(metricError)console.warn({event:"stock_publication_metric_write_failed",syncId:id,errorCode:metricError.code});await projectPartnerProductTransitions(id);}
  async fail(id:string,stage:StockSyncStage,page:number,error:unknown){const e=error as{code?:string;diagnostic?:{applicationMs?:number}};await createAdminClient().from("stock_sync_state").update({status:"failed",last_failed_sync_id:id,active_sync_id:null,failed_stage:stage,failed_page:page,error_category:"stock_sync_failure",database_error_code:e?.code??null,safe_error:"Exact stock synchronization failed.",publication_application_ms:e.diagnostic?.applicationMs??null,active_chunk_token:null,chunk_started_at:null,updated_at:new Date().toISOString()}).eq("id","exact_stock").eq("active_sync_id",id);}
...
function dbError(error:unknown,diagnostic?:Record<string,unknown>){const code=typeof error==="object"&&error&&"code" in error?String(error.code):undefined;return Object.assign(new Error("Stock persistence failed."),{code,diagnostic});}
function mapState(r:Record<string,unknown>):StockSyncState{return{status:r.status as StockSyncStatus,activeSyncId:str(r.active_sync_id),lastFailedSyncId:str(r.last_failed_sync_id),snapshotTime:str(r.snapshot_time),currentStage:r.current_stage as StockSyncStage|null,nextSkip:num(r.next_skip),pageSize:num(r.page_size),pagesProcessed:num(r.pages_processed),physicalRows:num(r.physical_rows),reservedRows:num(r.reserved_rows),incomingRows:num(r.incoming_rows),warehousesLoaded:num(r.warehouses_loaded),supplierBalanceRows:num(r.supplier_balance_rows),supplierBalanceGroups:num(r.supplier_balance_groups),supplierPositiveGroups:num(r.supplier_positive_groups),supplierNonpositiveExcluded:num(r.supplier_nonpositive_excluded),supplierOrdersRequested:num(r.supplier_orders_requested),productsMatched:num(r.products_matched),productsUnmatched:num(r.products_unmatched),rowsPublished:num(r.rows_published),rowsDeactivated:num(r.rows_deactivated),stockStagedRows:num(r.stock_staged_rows),arrivalsStagedRows:num(r.arrivals_staged_rows),stockDeltaUnchanged:num(r.stock_delta_unchanged),stockDeltaInserted:num(r.stock_delta_inserted),stockDeltaUpdated:num(r.stock_delta_updated),stockDeltaRemoved:num(r.stock_delta_removed),arrivalsDeltaUnchanged:num(r.arrivals_delta_unchanged),arrivalsDeltaInserted:num(r.arrivals_delta_inserted),arrivalsDeltaUpdated:num(r.arrivals_delta_updated),arrivalsDeltaRemoved:num(r.arrivals_delta_removed),publicationDbMs:nullableNum(r.publication_db_ms),publicationApplicationMs:nullableNum(r.publication_application_ms),publicationTimeoutBudgetMs:num(r.publication_timeout_budget_ms),publicationHeadroomPercent:nullableNum(r.publication_headroom_percent),publicationLockWaitMs:nullableNum(r.publication_lock_wait_ms),publicationTriggerRows:num(r.publication_trigger_rows),scanComplete:r.scan_complete===true,startedAt:str(r.started_at),lastSuccessfulSyncAt:str(r.last_successful_sync_at),errorCategory:str(r.error_category),failedStage:str(r.failed_stage),safeError:str(r.safe_error),failedPage:typeof r.failed_page==="number"?r.failed_page:null,updatedAt:String(r.updated_at)}};
...
function str(value:unknown){return typeof value==="string"?value:null;} function num(value:unknown){return typeof value==="number"?value:0;} function nullableNum(value:unknown){return typeof value==="number"?value:null;}
