...
export class SupabaseCatalogSnapshotWriter implements CatalogSnapshotWriter {
  async acquireLock(syncId: string, startedAt: string): Promise<boolean> {
    ...
    const { data, error: updateError } = await client.from("catalog_sync_state").update({ status: "running", active_sync_id: syncId, last_failed_sync_id: null, lock_acquired_at: startedAt, last_started_at: startedAt, error_category: null, failed_stage: null, updated_at: startedAt }).eq("id", "daily_catalog").or(`status.neq.running,lock_acquired_at.is.null,lock_acquired_at.lt.${staleBefore}`).select("id");
