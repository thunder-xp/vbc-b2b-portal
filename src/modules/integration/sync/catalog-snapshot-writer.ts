...
export type CatalogSnapshotWriteResult = { foldersUpserted: number; productsUpserted: number; rowsDeactivated: number; attributesUpserted?: number; attributesRemoved?: number };
export type CatalogSyncState = { status: string; rootName: string | null; lastSuccessfulSyncAt: string | null; durationMs: number | null; pagesProcessed: number; foldersReceived: number; productsReceived: number; foldersUpserted: number; productsUpserted: number; rowsDeactivated: number; errorCategory: string | null; failedStage: string | null; diagnostics?: NonNullable<CatalogSnapshotDTO["diagnostics"]> | null; nextScheduledRun: string };
...
export class SupabaseCatalogSnapshotWriter implements CatalogSnapshotWriter {
  ...
  async writeSnapshot(snapshot: CatalogSnapshotDTO, syncId: string): Promise<CatalogSnapshotWriteResult> {
    ...
    const attributePayload = snapshot.products.flatMap((product) => (product.attributes ?? []).flatMap((attribute) => { const productId = productIds.get(product.reference.externalId); return productId ? [{ product_id: productId, property_ref: attribute.propertyRef, attribute_key: attribute.key, label: attribute.label, raw_value: attribute.rawValue, display_value: attribute.displayValue, value_type: attribute.valueType, is_filterable: attribute.filterable, is_visible: attribute.visible, source_updated_at: product.metadata.sourceUpdatedAt, last_seen_sync_id: syncId, updated_at: new Date().toISOString() }] : []; }));
    let attributesUpserted = 0;
    for (const batch of chunks(attributePayload, BATCH_SIZE)) { const { data, error } = await client.from("catalog_product_attributes").upsert(batch, { onConflict: "product_id,property_ref" }).select("id"); if (error) throw new Error("Catalog product attribute batch failed."); attributesUpserted += data.length; }
    let attributesRemoved = 0;
    if (productIds.size) { const { data, error } = await client.from("catalog_product_attributes").delete().in("product_id", [...productIds.values()]).neq("last_seen_sync_id", syncId).select("id"); if (error) throw new Error("Catalog stale attribute cleanup failed."); attributesRemoved = data.length; }
