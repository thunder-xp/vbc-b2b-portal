...
describe("ChunkedPriceSyncService", () => {
  ...
  it("deduplicates a price page before calling the staging RPC", async () => {
    ...
    await new ChunkedPriceSyncService(provider, store).continue(syncId);
    expect(store.stagePrices).toHaveBeenCalledWith(
      syncId,
      [expect.objectContaining({ amount: 120 })],
      1,
      expect.objectContaining({
        received: 2,
        byPriceType: [expect.objectContaining({ received: 2, prepared: 1 })],
      }),
    );
...
function storeFixture(overrides: Partial<PriceSyncState> = {}) {
  const state: PriceSyncState = { status: "running", activeSyncId: syncId, lastFailedSyncId: null, startedAt: now, finishedAt: null, lastSuccessfulSyncAt: null, syncMode: "full_reconciliation", triggerKind: "scheduled", sourceWatermark: null, sourceQueryFrom: null, sourceInspectedThrough: null, runLatestSourcePeriod: null, lastSchedulerSeenAt: null, expectedNextRunAt: null, lastSourceSuccessAt: null, lastPublicationSuccessAt: null, nextRecoveryAttemptAt: null, recoveryAttemptCount: 0, lastFailureRetryable: false, currentStage: "price_type_scan", nextSkip: 0, pageSize: 500, pagesProcessed: 0, rowsScanned: 0, rowsStaged: 0, priceRowsReceived: 0, priceUniqueKeys: 0, priceDuplicateKeys: 0, priceRowsDeduplicated: 0, latestPricesResolved: 0, pricesPublished: 0, pricesDeactivated: 0, deltaUnchanged: 0, deltaInserted: 0, deltaUpdated: 0, deltaRemoved: 0, publicationBatches: 0, publicationDbDurationMs: 0, publicationTimeoutBudgetMs: 8_000, publicationHeadroomPercent: 100, publicationWarning: false, unmatchedProducts: 0, unknownPriceTypes: 0, scanComplete: false, errorCategory: null, failedStage: null, databaseErrorCode: null, safeError: null, failedPage: null, activeChunkToken: null, chunkStartedAt: null, lastPageStage: null, lastPageNumber: null, lastPageFingerprint: null, lastPageFirstKey: null, lastPageLastKey: null, retryCount: 0, odataRequestCount: 0, odataRequestDurationMs: 0, odataRequestDurationsMs: [], stagingDurationMs: 0, validationDurationMs: 0, publicationDurationMs: 0, continuationCount: 0, updatedAt: now, ...overrides };
  const store = {
    state,
    start: vi.fn(async () => ({ state, started: true })),
    heartbeat: vi.fn(async () => ({ recoveryRequired: false, recoveryAllowed: false, schedulerState: "FRESH" as const })),
