...
describe("DailyCatalogSyncService", () => {
  ...
  it("publishes complete automated NEW facts before the shared Public Retail projection", async () => {
    const writer = writerFixture();
    const orchestrator = {
      registerSourceRun: vi.fn(async () => undefined),
      completeSourceSync: vi.fn(async () => projection),
    ...
    const newProductProvider = { fetchSnapshot: vi.fn(async () => newProductSnapshot) };
    const newProductWriter = { publish: vi.fn(async () => newProductPublication) };
    const result = await new DailyCatalogSyncService(
      { fetchFullSnapshot: vi.fn(async () => snapshot) }, writer,
      orchestrator as never, newProductProvider as never, newProductWriter,
    ).runFullSync();
    expect(newProductProvider.fetchSnapshot).toHaveBeenCalledWith(["product"]);
    expect(newProductWriter.publish).toHaveBeenCalledWith(expect.any(String), newProductSnapshot, 1);
    expect(newProductWriter.publish.mock.invocationCallOrder[0]).toBeLessThan(orchestrator.completeSourceSync.mock.invocationCallOrder[0]!);
    expect(result.newProduct).toEqual(newProductPublication);
  });
...
const newProductSnapshot = { facts: [], businessDate: "2026-09-10", totalCatalogProducts: 1, totalCreationRequisiteRows: 0, totalEligibleReceipts: 0, totalEligibleReceiptLines: 0, creationRequisitePageCount: 1, receiptHeaderPageCount: 1, receiptLinePageCount: 0 };
const newProductPublication = { businessDate: "2026-09-10", totalProducts: 1, withSourceCreatedAt: 0, withoutSourceCreatedAt: 1, invalidSourceCreatedAt: 0, withMarketEntryAt: 0, withoutMarketEntryAt: 1, marketEntryBeforeCreationAnomalies: 0, futureMarketEntryCount: 0, new30: 0, new60: 0, new90: 0, new365: 0, automatedNewActivated: false };
