...
describe("PurchaseTemplateService", () => {
  ...
  beforeEach(() => {
    ...
    cart = { getCart: vi.fn(), getCheckoutIntent: vi.fn(), getItemCount: vi.fn(), addItem: vi.fn(), addItems: vi.fn(), updateQuantity: vi.fn(), removeItem: vi.fn(), mergeEstimateProducts: vi.fn(), previewEstimateProducts: vi.fn(), getEstimateSource: vi.fn().mockResolvedValue({ companyId: COMPANY, cartId: ORDER, lines: [{ productId: PRODUCT, sku: "400691", productName: "Camera", quantity: 2, partnerPrice: 10, currencyCode: "USD", priceUpdatedAt: "2026-07-31T00:00:00Z" }] }) };
