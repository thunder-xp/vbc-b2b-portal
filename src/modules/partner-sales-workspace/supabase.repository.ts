...
const estimateSchema = z.object({ name: z.string(), customer_name: z.string().nullable(), project_name: z.string().nullable(), status: z.enum(["draft", "ready", "sent", "accepted", "rejected", "archived"]), lifecycle_status: z.enum(["draft", "sent", "accepted"]), accepted_version_id: z.string().uuid().nullable() });
