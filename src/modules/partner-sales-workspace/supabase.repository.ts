...
const estimateSchema = z.object({ name: z.string(), customer_name: z.string().nullable(), project_name: z.string().nullable(), lifecycle_status: z.enum(["draft", "sent"]) });
...
export class SupabaseEstimateSalesOpportunityRepository implements EstimateSalesOpportunityRepository {
  async listCurrent(companyId: string, limit: number) {
    ...
    return parsed.data.map((row) => {
      ...
      versionStatus: row.status, estimateLifecycleStatus: estimate.lifecycle_status, sentAt: row.sent_at, createdAt: row.created_at,
