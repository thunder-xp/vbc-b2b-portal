  ...
  async listEstimateOpportunities(
    ...
    return [...latestByEstimate.values()].flatMap((source): PartnerEstimateSalesOpportunity[] => {
      ...
      if (
        ...
        return [toOpportunity(source, "accepted_ready_to_order", 1, source.acceptedAt)];
      }
      if (permissions.canSend && source.versionStatus === "prepared" && source.estimateLifecycleStatus === "draft" && source.readyDocumentId) {
        return [toOpportunity(source, "ready_to_send", 2, source.createdAt)];
      }
      if (source.versionStatus === "sent" && source.estimateLifecycleStatus === "sent" && source.sentAt) {
        return [toOpportunity(source, "awaiting_customer", 3, source.sentAt)];
      }
      return [];
...
function toOpportunity(source: Awaited<ReturnType<EstimateSalesOpportunityRepository["listCurrent"]>>[number], type: PartnerEstimateSalesOpportunity["type"], priority: 1 | 2 | 3, waitingSince: string): PartnerEstimateSalesOpportunity {
  return { id: `${type}:${source.versionId}`, type, priority, estimateId: source.estimateId, versionId: source.versionId, estimateNumber: source.estimateNumber,
    ...
    waitingSince, href: `/cabinet/estimates/${source.estimateId}${type === "accepted_ready_to_order" ? "#estimate-order-conversion" : ""}` };
}
