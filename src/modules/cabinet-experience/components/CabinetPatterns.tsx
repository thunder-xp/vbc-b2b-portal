...
export function AttentionActionItem({ action, fields, title, detail, status, Icon, priority = "IMPORTANT_UPDATE" }: { action: (formData: FormData) => void | Promise<void>; fields: Readonly<Record<string, string>>; title: string; detail: string; status: string; Icon: LucideIcon; priority?: "ACTION_REQUIRED" | "IMPORTANT_UPDATE" | "INFORMATIONAL" }) {
  const urgent = priority === "ACTION_REQUIRED";
  return <form action={action}>{Object.entries(fields).map(([name, value]) => <input key={name} name={name} type="hidden" value={value} />)}<button className={`group flex min-h-16 w-full items-center gap-3 rounded-xl border px-4 py-3 text-left focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald-700 ${urgent ? "border-amber-200 bg-amber-50 hover:border-amber-300" : "border-emerald-200 bg-emerald-50/60 hover:border-emerald-300"}`} type="submit"><span className={`flex size-10 shrink-0 items-center justify-center rounded-full bg-white ${urgent ? "text-amber-800" : "text-emerald-800"}`}><Icon aria-hidden className="size-5" /></span><span className="min-w-0 flex-1"><span className="block text-sm font-semibold text-zinc-950">{title}</span><span className="block text-sm text-zinc-600">{detail}</span></span><span className={`hidden text-xs font-semibold sm:block ${urgent ? "text-amber-900" : "text-emerald-900"}`}>{status}</span></button></form>;
}
