...
export function CampaignBuilder({ options }: { options: CampaignBuilderOptions }) {
  ...
  return <section className="rounded-md border border-zinc-200 bg-white p-5">
    ...
    <div className="mt-6 min-h-80">
      ...
      {step === 1 ? <fieldset><legend className="font-semibold">Товары из локального каталога</legend><p className="mt-1 text-sm text-zinc-600">Цены не сохраняются в кампании и разрешаются из актуальной коммерческой модели.</p><div className="mt-4 grid max-h-96 gap-2 overflow-y-auto sm:grid-cols-2">{options.products.map((product) => <Check checked={products.includes(product.id)} key={product.id} label={<><b>{product.sku}</b> · {product.name}</>} onChange={() => toggle(product.id, products, setProducts)} />)}</div></fieldset> : null}
      {step === 2 ? <div><fieldset><legend className="font-semibold">Управляемое правило аудитории</legend><div className="mt-3 grid gap-2">{([['explicit_company','Выбранные компании'],['all_active_partners','Все активные партнёры'],['commercial_mode_full','Полный коммерческий доступ'],['commercial_mode_retail_only','Только розничные цены']] as const).map(([value,label]) => <label className="flex min-h-11 items-center gap-3" key={value}><input checked={audienceMode === value} name="audience" onChange={() => setAudienceMode(value)} type="radio" />{label}</label>)}</div></fieldset>{audienceMode === "explicit_company" ? <fieldset className="mt-4"><legend className="font-semibold">Компании</legend><div className="mt-2 grid max-h-72 gap-2 overflow-y-auto sm:grid-cols-2">{options.companies.map((company) => <Check checked={companies.includes(company.id)} key={company.id} label={company.name} onChange={() => toggle(company.id, companies, setCompanies)} />)}</div></fieldset> : null}</div> : null}
