...
export function OrderDateChangeDialog({ orderHistoryId, currentDate }: { orderHistoryId: string; currentDate: string }) {
  ...
  return <>
    <button className="inline-flex min-h-11 items-center gap-2 rounded-md border border-emerald-700 px-3 text-sm font-semibold text-emerald-800" onClick={() => {
      dialogRef.current?.showModal();
      recordBehaviorInteraction({ eventName: "date_change_started", route: "/cabinet/reservation-requests", sourceSurface: "planned_shipments" });
    }} type="button">
    ...
    <dialog className="w-[min(32rem,calc(100%-2rem))] rounded-lg p-0 backdrop:bg-zinc-950/40" ref={dialogRef}>
      <form action={action} className="space-y-4 p-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <button aria-label="Закрыть" className="inline-flex size-11 items-center justify-center rounded text-zinc-500 hover:bg-zinc-100" onClick={() => dialogRef.current?.close()} type="button"><X className="size-5" /></button>
        </div>
