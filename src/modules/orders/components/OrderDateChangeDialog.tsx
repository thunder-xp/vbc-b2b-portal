...
import {
  cancelOrderDateChangeRequestAction,
  createOrderDateChangeRequestAction,
} from "../actions/order.actions";
import type { OrderDateChangeRequest } from "../types";

const initialState: ActionResult<OrderDateChangeRequest | null> = {
  success: true,
  errorCode: null,
  message: "",
  data: null,
...
export function OrderDateChangeDialog({
  orderHistoryId,
  currentDate,
}: {
  ...
  return (
    <>
      <button
        className="inline-flex min-h-11 items-center gap-2 rounded-md border border-emerald-700 px-3 text-sm font-semibold text-emerald-800"
        onClick={() => {
          dialogRef.current?.showModal();
          recordBehaviorInteraction({
            eventName: "date_change_started",
      ...
      >
        ...
        {copy.requestDateChange}
      </button>
      <dialog
        className="w-[min(32rem,calc(100%-2rem))] rounded-lg p-0 backdrop:bg-zinc-950/40"
        ref={dialogRef}
      >
        <form action={action} className="space-y-4 p-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h2 className="text-lg font-semibold">{copy.dateChangeTitle}</h2>
            ...
            <button
              ...
              className="inline-flex size-11 items-center justify-center rounded text-zinc-500 hover:bg-zinc-100"
          ...
          <label className="block text-sm font-medium">
            {copy.currentDate}
            <input
              className="mt-1 h-10 w-full rounded-md border border-zinc-200 bg-zinc-50 px-3"
              readOnly
              value={currentDate}
            />
          </label>
          ...
          <label className="block text-sm font-medium">
            ...
            <textarea
              className="mt-1 min-h-24 w-full rounded-md border border-zinc-300 p-3"
              maxLength={1000}
              name="comment"
            />
          </label>
          ...
          <div className="flex flex-wrap justify-end gap-3">
            ...
            <button
              ...
              type="submit"
            >
              {pending ? copy.submitting : copy.sendRequest}
            </button>
          </div>
        </form>
      </dialog>
...
export function CancelOrderDateChangeButton({
  requestId,
}: {
  requestId: string;
}) {
  ...
  return (
    <button
      ...
      onClick={() =>
        startTransition(
          () => void cancelOrderDateChangeRequestAction(requestId),
        )
      }
      type="button"
    >
