...
export function SpecificationDetail({ specification }: { specification: ProjectSpecificationDetailDto }) {
  ...
  const showPartnerCommercial =
    "partnerPurchaseTotal" in specification.totals;
  return <div className="space-y-6">
    ...
    <section className="overflow-hidden rounded-lg border border-zinc-200 bg-white"><div className="border-b border-zinc-200 px-5 py-4"><h2 className="font-semibold">Оборудование</h2></div>{specification.lines.length ? <div className="overflow-x-auto"><table className="w-full min-w-[860px] text-left text-sm"><thead className="bg-zinc-50 text-xs uppercase text-zinc-500"><tr><th className="px-4 py-3">Модель</th><th className="px-4 py-3">Количество</th>{showPartnerCommercial ? <th className="px-4 py-3">Партнёрская</th> : null}<th className="px-4 py-3">Розница</th><th className="px-4 py-3">Наличие</th><th className="px-4 py-3">Поступление</th><th className="px-4 py-3">Итого</th></tr></thead><tbody className="divide-y divide-zinc-100">{specification.lines.map((line) => <tr key={line.id}><td className="px-4 py-4"><Link className="font-semibold text-zinc-950 hover:text-emerald-700" href={`/cabinet/catalog/${line.slug}`}>{line.productName}</Link><div className="mt-1 text-xs text-zinc-500">SKU {line.sku}</div></td><td className="px-4 py-4">{isDraft ? <SpecificationItemControls itemId={line.id} quantity={line.quantity} specificationId={specification.id} /> : line.quantity}</td>{showPartnerCommercial ? <td className="px-4 py-4">{line.partnerUnitPrice ?? "Уточняется"}</td> : null}<td className="px-4 py-4">{line.retailUnitPrice ?? "Уточняется"}</td><td className="px-4 py-4">{line.availableStock === null ? "Уточняется" : line.availableStock}</td><td className="px-4 py-4">{line.nearestArrivalDate ? <><div>{line.nearestArrivalDate}</div><div className="text-xs text-zinc-500">{line.nearestArrivalQuantity ?? "—"} шт.</div></> : "—"}</td><td className="px-4 py-4">{showPartnerCommercial ? <><div className="font-semibold">{line.partnerLineTotal ?? "—"}</div><div className="text-xs text-zinc-500">Розница: {line.retailLineTotal ?? "—"}</div></> : <div className="font-semibold">{line.retailLineTotal ?? "—"}</div>}</td></tr>)}</tbody></table></div> : <p className="px-5 py-10 text-center text-sm text-zinc-500">Добавьте оборудование из каталога ниже.</p>}</section>
    <SpecificationTotals showPartnerCommercial={showPartnerCommercial} totals={specification.totals} />
    {isDraft && <SubmitSpecificationButton disabled={!specification.lines.length} specificationId={specification.id} />}
  </div>;
}

function SpecificationTotals({ totals, showPartnerCommercial }: { totals: ProjectSpecificationDetailDto["totals"]; showPartnerCommercial: boolean }) {
  const values = showPartnerCommercial
    ? [["Закупка партнёра", totals.partnerPurchaseTotal], ["Розничная сумма", totals.retailTotal], ["Потенциальная валовая прибыль", totals.potentialGrossProfit], ["Наценка", totals.markupPercentage]]
    : [["Розничная сумма", totals.retailTotal]];
