...
export function StatusBadge({ status }: { status: ProjectSpecificationStatus }) {
  return <span className={`w-fit rounded-full px-3 py-1 text-xs font-semibold ${status === ProjectSpecificationStatus.Draft ? "bg-amber-100 text-amber-800" : "bg-emerald-100 text-emerald-800"}`}>{status === ProjectSpecificationStatus.Draft ? "Черновик" : "Отправлена"}</span>;
}
