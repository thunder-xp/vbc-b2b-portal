...
export function ProductComparisonView({ categoryId, companyId, userId }: { categoryId: string; companyId: string; userId: string }) {
  ...
  const partnerPricingVisible = comparison.products.some((product) =>
    Boolean(views.get(product.id)?.partnerPrice),
  );
  return <div className="overflow-x-auto"><table className="min-w-[720px] border-collapse text-left text-sm"><caption className="sr-only">Сравнение товаров</caption><thead><tr><th className="border-b border-zinc-200 p-3 text-zinc-500">Параметр</th>{comparison.products.map((product) => <th className="border-b border-zinc-200 p-3" key={product.id}><Link className="font-semibold text-emerald-700" href={`/cabinet/catalog/${product.slug}`}>{product.name}</Link><span className="mt-1 block text-xs font-normal text-zinc-500">Артикул: {product.sku}</span></th>)}</tr></thead><tbody><Row label={partnerPricingVisible ? "Партнёрская цена" : "Розничная цена"} values={comparison.products.map((product) => { const view = views.get(product.id); return (partnerPricingVisible ? view?.partnerPrice : view?.retailPrice)?.formattedAmount ?? "Уточняется"; })} /><Row label="Наличие" values={comparison.products.map((product) => { const value = views.get(product.id)?.stock?.exactAvailableQuantity; return value === null || value === undefined ? "Уточняется" : `${value} шт.`; })} />{uniqueCharacteristics(comparison.products).map((label) => <Row key={label} label={label} values={comparison.products.map((product) => product.keyCharacteristics.find((item) => item.label === label)?.value ?? "—")} />)}</tbody></table></div>;
}
