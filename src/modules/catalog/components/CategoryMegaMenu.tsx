...
export function CategoryMegaMenu({ categories }: { categories: CatalogCategoryDto[] }) {
  ...
  return (
    <div className="relative">
      <button aria-expanded={open} className="inline-flex h-11 items-center gap-2 rounded-md bg-emerald-700 px-4 text-sm font-semibold text-white hover:bg-emerald-800" onClick={() => setOpen((value) => !value)} type="button">
        <Menu aria-hidden="true" className="size-4" /> Категории
      </button>
      {open && (
        <div className="fixed inset-0 z-40 bg-white lg:absolute lg:inset-auto lg:left-0 lg:top-12 lg:w-[min(900px,calc(100vw-3rem))] lg:rounded-lg lg:border lg:border-zinc-200 lg:shadow-xl">
          ...
          <div className="grid max-h-[calc(100vh-3.5rem)] overflow-auto p-3 lg:grid-cols-3 lg:gap-4 lg:p-5">
            <div className={`${direction ? "hidden" : "block"} lg:block`}><CategoryColumn items={tree} onChoose={(id) => { setDirectionId(id); setCategoryId(null); }} selectedId={directionId} /></div>
            <div className={`${direction && !category ? "block" : "hidden"} lg:block`}>
              {direction ? <CategoryColumn items={direction.children} onChoose={setCategoryId} selectedId={categoryId} /> : <MenuHint text="Выберите направление" />}
            </div>
...
function CategoryColumn({ items, onChoose, selectedId }: { items: CatalogCategoryNode[]; onChoose: (id: string) => void; selectedId: string | null }) {
  ...
  ) : <Link className="block rounded-md px-3 py-2 text-sm hover:bg-zinc-50" href={`/cabinet/catalog?category=${item.id}`} key={item.id}>{item.name}</Link>)}</div>;
}
