...
export function ProductCard({ capabilities, commercialView, priceTypeName, product }: ProductCardProps) {
  ...
  return <article className="flex h-full flex-col rounded-lg border border-zinc-200 bg-white shadow-sm transition hover:border-emerald-500">
    ...
    <div className="flex flex-1 flex-col p-4">
      ...
      {product.keyCharacteristics.length > 0 && <div className="mt-3 flex flex-wrap gap-1.5">{product.keyCharacteristics.slice(0, 2).map((item) => <span className="rounded-full bg-zinc-100 px-2 py-1 text-xs font-medium text-zinc-700" key={item.label}>{item.value}</span>)}</div>}
