...
export function ProductCard({ analyticsEventName, analyticsSurface, cartSuccessEventName, capabilities, commercialView, companyId = null, contextLine, favorite = false, imagePriority = false, product, userId = null }: ProductCardProps) {
  ...
  return <CatalogProductCardFrame
    ...
    title={<Link className="line-clamp-2 h-10 rounded-sm text-sm font-semibold leading-5 text-zinc-950 outline-none hover:text-emerald-700 focus-visible:ring-2 focus-visible:ring-emerald-500" href={`/cabinet/catalog/${product.slug}`} prefetch={false} title={product.name}>{product.name}</Link>}
  />;
}
