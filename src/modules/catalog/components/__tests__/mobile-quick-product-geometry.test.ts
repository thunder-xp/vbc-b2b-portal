...
describe("mobile quick product geometry and boundaries", () => {
  it("keeps search, cart, quantity, and add controls at least 44px high", () => {
  ...
  it("uses bounded server catalog results and canonical cart mutation without browser database access", () => {
    expect(route).toContain("listCatalogProductsAction");
    expect(route).toContain("const RESULT_LIMIT = 8");
    expect(component).toContain("addToCartAction(product.id, quantity)");
