...
describe("product card ergonomics contracts", () => {
  it("keeps deterministic identity, commercial, availability, and action zones", () => {
    ...
    expect(card).toContain("mt-auto pt-3");
    expect(card).toContain("min-h-11 justify-end");
  });

  it("uses restrained hover and visible focus without changing border geometry", () => {
    expect(card).toContain("transition-shadow hover:shadow-md");
    expect(card).toContain("focus-within:ring-2");
    expect(card).not.toContain("hover:border-emerald");
  });
