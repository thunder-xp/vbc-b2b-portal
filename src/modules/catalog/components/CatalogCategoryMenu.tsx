...
export function CatalogCategoryMenu<T extends CatalogCategoryMenuItem>({
  ...
  return <div className="relative" ref={containerRef}>
    <button
      ...
      aria-expanded={open}
      aria-haspopup="menu"
      className={`inline-flex h-11 items-center gap-2 bg-emerald-700 px-4 text-sm font-semibold text-white hover:bg-emerald-800 ${square ? "" : "rounded-md"}`}
    ...
    >
      ...
      <div className="grid max-h-[calc(100vh-3.5rem)] overflow-auto p-3 lg:grid-cols-3 lg:gap-4 lg:p-5">
        <div className={`${direction ? "hidden" : "block"} lg:block`}><CategoryColumn categoryHref={categoryHref} items={tree} onChoose={(id) => { setDirectionId(id); setCategoryId(null); }} onNavigate={() => setOpen(false)} selectedId={directionId} /></div>
        <div className={`${direction && !category ? "block" : "hidden"} lg:block`}>
          {direction ? <CategoryColumn categoryHref={categoryHref} items={direction.children} onChoose={setCategoryId} onNavigate={() => setOpen(false)} selectedId={categoryId} /> : <MenuHint text={labels.selectDirection} />}
        </div>
        <div className={`${category ? "block" : "hidden"} lg:block`}>
          {category ? <CategoryLinks categoryHref={categoryHref} items={category.children.length ? category.children : [category]} onNavigate={() => setOpen(false)} /> : <MenuHint text={labels.selectCategory} />}
        </div>
      </div>
    </div> : null}
  </div>;
...
function CategoryColumn<T extends CatalogCategoryMenuItem>({ categoryHref, items, onChoose, onNavigate, selectedId }: { categoryHref: (category: CatalogCategoryNode<T>) => string; items: CatalogCategoryNode<T>[]; onChoose: (id: string) => void; onNavigate: () => void; selectedId: string | null }) {
  return <div className="space-y-1">{items.map((item) => item.children.length ? (
    <button className={`flex min-h-11 w-full items-center justify-between px-3 py-2 text-left text-sm ${selectedId === item.id ? "bg-emerald-50 font-semibold text-emerald-800" : "hover:bg-zinc-50"}`} key={item.id} onClick={() => onChoose(item.id)} type="button"><span>{item.name}</span><ChevronRight aria-hidden="true" className="size-4" /></button>
...
function CategoryLinks<T extends CatalogCategoryMenuItem>({ categoryHref, items, onNavigate }: { categoryHref: (category: CatalogCategoryNode<T>) => string; items: CatalogCategoryNode<T>[]; onNavigate: () => void }) {
  return <div className="space-y-1">{items.map((item) => <Link className="flex min-h-11 items-center px-3 py-2 text-sm hover:bg-emerald-50 hover:text-emerald-800" href={categoryHref(item)} key={item.id} onClick={onNavigate} prefetch={false}>{item.name}</Link>)}</div>;
}
