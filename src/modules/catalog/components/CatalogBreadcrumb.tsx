...
export function CatalogBreadcrumb({ categories, selectedId }: { categories: CatalogCategoryDto[]; selectedId?: string }) {
  ...
  return <nav aria-label="Хлебные крошки" className="flex flex-wrap items-center gap-2 text-sm text-zinc-500"><Link href="/cabinet/catalog">Каталог</Link>{path.map((item) => <span className="flex items-center gap-2" key={item.id}><span>/</span><Link className="text-zinc-700 hover:text-emerald-700" href={`/cabinet/catalog?category=${item.id}`}>{item.name}</Link></span>)}</nav>;
}
