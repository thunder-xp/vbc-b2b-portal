...
export function ExpandableDescription({ text }: { text: string }) {
  ...
  return <div className="mt-3.5">
    ...
    {canExpand || expanded ? <button aria-expanded={expanded} className="mt-2 text-sm font-semibold text-emerald-700 hover:text-emerald-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald-600" onClick={() => setExpanded((value) => !value)} type="button">{expanded ? copy.collapse : copy.more}</button> : null}
  </div>;
}
