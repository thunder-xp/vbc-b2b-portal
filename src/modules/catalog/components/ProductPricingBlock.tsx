...
export function ProductPricingBlock({ commercialView, freshness, variant = "card" }: { commercialView?: ProductCommercialViewDto; freshness?: FreshnessView | null; variant?: "card" | "detail" }) {
  ...
  if (variant === "card") return <div className={`grid ${showPartnerPrice ? "grid-cols-2 divide-x" : "grid-cols-1"} divide-zinc-200 rounded-md border border-zinc-200 bg-zinc-50`}>{showPartnerPrice ? <PriceColumn label="ПАРТНЁРСКАЯ" value={commercialView?.partnerPrice?.formattedAmount} /> : null}<PriceColumn label="РОЗНИЧНАЯ" value={commercialView?.retailPrice?.formattedAmount} /></div>;
...
function PriceColumn({ label, value }: { label: string; value?: string | null }) { return <div className="min-w-0 px-2 py-2"><p className="truncate text-[10px] font-semibold text-zinc-500">{label}</p><p className="mt-0.5 truncate text-sm font-semibold text-zinc-950" title={value ?? "Цена уточняется"}>{value ?? "Цена уточняется"}</p></div>; }
