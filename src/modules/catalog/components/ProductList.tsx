...
export function ProductList({ capabilities, catalogState, commercialViews = {}, companyId, contextBadge, favoriteProductIds = [], locale = "ru", products, userId }: {
  ...
  return <div className="divide-y divide-zinc-200 overflow-hidden rounded-md border border-zinc-200 bg-white">
    {products.map((product, index) => {
      ...
      return <article className="grid min-w-0 grid-cols-[64px_minmax(0,1fr)] gap-3 p-3 xl:grid-cols-[72px_minmax(180px,1fr)_minmax(150px,0.72fr)_minmax(130px,0.55fr)] xl:items-center min-[1360px]:grid-cols-[72px_minmax(180px,1fr)_minmax(150px,0.72fr)_minmax(130px,0.55fr)_minmax(320px,auto)]" key={product.id}>
        ...
        {capabilities.showPrice ? <div className="col-span-2 flex h-[5.25rem] items-center xl:col-span-1"><ProductPricingBlock commercialView={commercialView} locale={locale} showPartnerPrice={capabilities.showPartnerPrice} showRetailPrice={capabilities.showRetailPrice} /></div> : null}
        {capabilities.showStock ? <div className="col-span-2 flex h-[5.25rem] min-w-0 items-center xl:col-span-1"><ProductAvailabilityBlock locale={locale} stock={commercialView?.stock} /></div> : null}
        <div className="col-span-2 flex min-h-11 flex-wrap items-center justify-end gap-2 xl:max-[1359px]:col-span-4 min-[1360px]:col-span-1 min-[1360px]:min-h-[5.25rem] min-[1360px]:flex-nowrap" data-testid="catalog-list-actions">
