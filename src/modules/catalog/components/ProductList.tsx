  ...
  return <div className="divide-y divide-zinc-200 overflow-hidden rounded-md border border-zinc-200 bg-white">
    {products.map((product, index) => {
      ...
      return <article className="grid min-w-0 grid-cols-[64px_minmax(0,1fr)] gap-2.5 p-2.5 xl:items-center xl:max-[1439px]:grid-cols-[64px_minmax(180px,1fr)_minmax(140px,0.65fr)_minmax(120px,0.5fr)] min-[1440px]:grid-cols-[64px_minmax(6rem,1fr)_minmax(7.5rem,0.62fr)_minmax(6.5rem,0.5fr)_minmax(21.75rem,auto)]" key={product.id}>
        <Link className="relative size-16 self-center overflow-hidden rounded bg-zinc-100 min-[1440px]:col-start-1 min-[1440px]:row-start-1" href={productHref} prefetch={false}>
        ...
        <div className="min-w-0 self-center min-[1440px]:col-start-2 min-[1440px]:row-start-1"><p className="text-[11px] font-medium uppercase text-zinc-500">SKU {product.sku}</p><Link className="mt-0.5 line-clamp-2 text-sm font-semibold leading-5 text-zinc-950 hover:text-emerald-700" href={productHref} prefetch={false} title={product.name}>{product.name}</Link></div>
        ...
        {capabilities.showStock ? <div className="col-span-2 flex h-16 min-w-0 items-center overflow-hidden rounded-md xl:col-span-1 min-[1440px]:col-start-4 min-[1440px]:row-start-1"><ProductAvailabilityBlock locale={locale} stock={commercialView?.stock} /></div> : null}
        ...
        <div className="col-span-2 flex min-h-11 flex-wrap items-center justify-end gap-2 xl:max-[1439px]:col-span-4 min-[1440px]:col-span-1 min-[1440px]:col-start-5 min-[1440px]:row-start-1 min-[1440px]:h-16 min-[1440px]:flex-nowrap" data-testid="catalog-list-actions">
