...
export function ProductList({ capabilities, commercialViews = {}, companyId, favoriteProductIds = [], products, userId }: {
  ...
  return <div className="divide-y divide-zinc-200 overflow-hidden rounded-md border border-zinc-200 bg-white">
    {products.map((product, index) => {
      ...
      return <article className="grid min-w-0 grid-cols-[64px_minmax(0,1fr)] gap-3 p-3 md:grid-cols-[72px_minmax(180px,1fr)_minmax(190px,0.8fr)_minmax(150px,0.6fr)_auto] md:items-center" key={product.id}>
        ...
        <div className="min-w-0"><MerchandisingBadges labels={product.merchandisingLabels} /><p className="mt-1 text-[11px] font-medium uppercase text-zinc-500">SKU {product.sku}</p><Link className="mt-0.5 line-clamp-2 text-sm font-semibold text-zinc-950 hover:text-emerald-700" href={`/cabinet/catalog/${product.slug}`} prefetch={false}>{product.name}</Link></div>
