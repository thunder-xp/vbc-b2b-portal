...
export function ProductSpecificationAction({ compact = false, productId }: { compact?: boolean; productId: string }) {
  ...
  return <div className="relative">
    <button aria-label={compact ? "Добавить в смету" : undefined} aria-expanded={open} className={`inline-flex items-center justify-center gap-2 rounded-md border text-xs font-semibold outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 ${open ? "border-emerald-300 bg-emerald-50 text-emerald-800" : "border-zinc-300 bg-white text-zinc-800 hover:bg-zinc-50"} ${compact ? "size-11 p-0" : "h-11 px-3"}`} onClick={toggle} title={compact ? "Добавить в смету" : undefined} type="button"><ClipboardList aria-hidden="true" className="size-4" />{compact ? null : "В смету"}</button>
    {open ? <div className={`absolute z-20 mt-2 w-[min(22rem,calc(100vw-2rem))] rounded-md border border-zinc-200 bg-white p-4 shadow-lg ${compact ? "right-0" : "left-0"}`}>
    ...
    {compact && message ? <p aria-live="polite" className="fixed bottom-4 left-1/2 z-50 max-w-[calc(100vw-2rem)] -translate-x-1/2 rounded-md bg-zinc-950 px-3 py-2 text-xs font-medium text-white shadow-lg">{message}</p> : null}
  </div>;
}
