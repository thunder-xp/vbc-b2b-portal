...
export function ProductSpecificationAction({ compact = false, productId }: { compact?: boolean; productId: string }) {
  ...
  return <div className="relative">
    <button aria-label={compact ? "Добавить в смету" : undefined} aria-expanded={open} className={`inline-flex h-9 items-center justify-center gap-2 rounded-md border border-zinc-300 bg-white text-xs font-semibold text-zinc-800 hover:bg-zinc-50 ${compact ? "size-9 p-0" : "px-3"}`} onClick={toggle} title={compact ? "Добавить в смету" : undefined} type="button"><ClipboardList aria-hidden="true" className="size-4" />{compact ? null : "В смету"}</button>
