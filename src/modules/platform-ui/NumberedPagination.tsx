...
export function NumberedPagination({
  ...
  return (
    <nav aria-label={ariaLabel} className="flex flex-wrap items-center justify-center gap-1.5 border-t border-zinc-200 pt-5">
      {current > 1 ? <Link aria-label={previousAriaLabel} className={linkClassName} href={hrefForPage(current - 1)} prefetch={false}>{previousLabel}</Link> : <span aria-disabled="true" className={disabledClassName}>{previousLabel}</span>}
      ...
      {current < totalPages ? <Link aria-label={nextAriaLabel} className={linkClassName} href={hrefForPage(current + 1)} prefetch={false}>{nextLabel}</Link> : <span aria-disabled="true" className={disabledClassName}>{nextLabel}</span>}
    </nav>
  );
}
