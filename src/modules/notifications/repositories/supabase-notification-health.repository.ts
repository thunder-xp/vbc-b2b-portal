...
const healthSchema = z.object({
  ...
  gateway: z.object({
    ...
    rateLimits: z.object({
      recipientPerHour: z.number().int().positive(),
      companyPurposeChannelPerHour: z.number().int().positive(),
      workerBatchDefault: z.number().int().positive(),
      workerBatchMaximum: z.number().int().positive(),
      allowedLast24Hours: z.number().int().nonnegative(),
      limitedLast24Hours: z.number().int().nonnegative(),
    }),
    governanceCounts: z.array(z.object({
      purpose: z.string(),
      channel: z.enum(["email", "in_app", "sms", "telegram"]),
      decision: z.enum(["ALLOW", "SUPPRESS"]),
      reason: z.string(),
    ...
    byCommunicationTypeChannel: z.array(z.object({
      ...
      purpose: z.string(),
      channel: z.enum(["email", "in_app", "sms", "telegram"]),
      mode: z.enum(["DISABLED", "DRY_RUN", "SANDBOX", "LIVE"]),
    ...
    recentDeliveries: z.array(z.object({
      ...
      channel: z.enum(["email", "in_app", "sms", "telegram"]),
      mode: z.enum(["DISABLED", "DRY_RUN", "SANDBOX", "LIVE"]),
      requestedMode: z.enum(["DISABLED", "DRY_RUN", "SANDBOX", "LIVE"]),
      effectiveMode: z.enum(["DISABLED", "DRY_RUN", "SANDBOX", "LIVE"]),
      policyDecision: z.enum(["ALLOW", "SUPPRESS"]),
      preferenceResult: z.enum(["ALLOWED", "SUPPRESSED", "NOT_APPLICABLE", "NOT_CONFIGURED"]),
      rateLimitResult: z.enum(["ALLOWED", "RATE_LIMITED", "NOT_EVALUATED"]),
      sandboxResult: z.enum(["NOT_APPLICABLE", "ALLOWED", "RECIPIENT_NOT_ALLOWED", "PROVIDER_UNAVAILABLE"]),
      recipient: z.string(),
