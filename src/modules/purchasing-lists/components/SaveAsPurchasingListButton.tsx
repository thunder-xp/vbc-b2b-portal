...
export function SaveAsPurchasingListButton({ source, orderId, selections, label = "Сохранить как список закупок" }: { source: "cart" | "order" | "quick_reorder"; orderId?: string; selections?: Array<{ lineId: string; quantity: number }>; label?: string }) {
  ...
  return <><button className="inline-flex min-h-11 items-center justify-center gap-2 rounded-md border border-zinc-300 bg-white px-3 py-2 text-sm font-semibold text-zinc-800" onClick={() => setOpen(true)} type="button"><BookmarkPlus className="size-4" />{label}</button>
