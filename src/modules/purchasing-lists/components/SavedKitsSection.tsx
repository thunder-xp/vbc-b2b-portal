...
import { Archive, ArrowRight, Copy, Layers3, LoaderCircle, Minus, Pencil, Plus, Save, X } from "lucide-react";
...
export function SavedKitsSection({
  ...
  function renameKit() {
    if (!detail || !renameValue.trim() || pending || !detail.canManage) return;
    startTransition(async () => {
      const result = await updatePurchasingListMetadataAction(detail.id, detail.revision, {
        name: renameValue,
      ...
      setDetail({ ...detail, name: renameValue.trim(), revision: result.data.revision });
      setMessage(copy.savedChanges);
      setShowRename(false);
      void refreshKits();
    });
  ...
  function archiveKit() {
    if (!detail || pending || !detail.canManage) return;
    startTransition(async () => {
      const result = await setPurchasingListArchivedAction(detail.id, detail.revision, true);
      if (!result.success) {
        setMessage(copy.saveFailed);
        return;
      }
      setDetail(null);
      setMessage(copy.archived);
      void refreshKits();
    });
  }
    ...
    {detail ? <div className="fixed inset-0 z-50 bg-black/45" role="presentation">
      <section aria-label={detail.name} aria-modal="true" className="absolute inset-x-0 bottom-0 flex max-h-[92dvh] flex-col rounded-t-2xl bg-zinc-50 pb-[env(safe-area-inset-bottom)] shadow-2xl sm:inset-y-0 sm:left-auto sm:right-0 sm:max-h-none sm:w-[36rem] sm:rounded-none" data-testid="saved-kit-composition" role="dialog">
        <header className="flex items-start justify-between gap-3 border-b border-zinc-200 bg-white px-4 py-3">
          <div><p className="text-xs font-semibold uppercase text-emerald-700">{copy.title}</p><h2 className="font-semibold text-zinc-950">{detail.name}</h2><p className="mt-1 text-xs text-zinc-600"><strong>{detail.itemCount}</strong> {copy.products} · <strong>{detail.totalQuantity}</strong> {copy.units}</p><p className="mt-1 text-xs text-zinc-600"><strong>{detail.readyCount}</strong> {copy.ready} · <strong>{detail.attentionCount}</strong> {copy.attention}</p></div>
          <button aria-label={copy.close} className="inline-flex size-11 shrink-0 items-center justify-center rounded-md text-zinc-600" onClick={() => { setDetail(null); setMessage(null); setShowSaveAsNew(false); setShowRename(false); }} type="button"><X aria-hidden="true" className="size-5" /></button>
        </header>
        ...
        <footer className="space-y-2 border-t border-zinc-200 bg-white p-4">
          ...
          <button className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-md bg-emerald-700 px-4 text-sm font-semibold text-white disabled:bg-zinc-300 disabled:text-zinc-600" disabled={!canSelectProducts || detail.readyCount === 0 || pending} onClick={useKit} type="button"><Layers3 aria-hidden="true" className="size-4" />{pending ? copy.refreshing : copy.useKit}</button>
          <p className="text-center text-xs text-zinc-500">{copy.currentTruth}</p>
          {canManage && detail.canManage ? <div className="flex flex-col gap-2 sm:grid sm:grid-cols-2">
            <button className="inline-flex min-h-11 items-center justify-center gap-2 rounded-md border border-zinc-300 bg-white px-3 text-sm font-semibold text-zinc-700 disabled:text-zinc-400" disabled={pending} onClick={saveChanges} type="button"><Save aria-hidden="true" className="size-4" />{copy.saveChanges}</button>
            <button className="inline-flex min-h-11 items-center justify-center gap-2 rounded-md border border-zinc-300 bg-white px-3 text-sm font-semibold text-zinc-700 disabled:text-zinc-400" disabled={pending} onClick={() => setShowSaveAsNew((value) => !value)} type="button"><Copy aria-hidden="true" className="size-4" />{copy.saveAsNew}</button>
            <button className="inline-flex min-h-11 items-center justify-center gap-2 rounded-md border border-zinc-300 bg-white px-3 text-sm font-semibold text-zinc-700 disabled:text-zinc-400" disabled={pending} onClick={() => { setRenameValue(detail.name); setShowRename((value) => !value); }} type="button"><Pencil aria-hidden="true" className="size-4" />{copy.rename}</button>
            <button className="inline-flex min-h-11 items-center justify-center gap-2 rounded-md border border-rose-200 bg-white px-3 text-sm font-semibold text-rose-700 disabled:text-zinc-400" disabled={pending} onClick={archiveKit} type="button"><Archive aria-hidden="true" className="size-4" />{copy.archive}</button>
          </div> : null}
          ...
          {showRename ? <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-2"><input aria-label={copy.rename} className="h-11 min-w-0 rounded-md border border-zinc-300 px-3 text-sm" maxLength={120} onChange={(event) => setRenameValue(event.target.value)} value={renameValue} /><button className="min-h-11 rounded-md bg-zinc-900 px-4 text-sm font-semibold text-white disabled:bg-zinc-300" disabled={!renameValue.trim() || pending} onClick={renameKit} type="button">{copy.save}</button></div> : null}
        </footer>
      </section>
    </div> : null}
  </section>;
