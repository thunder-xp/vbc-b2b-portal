...
describe("SavedKitsSection", () => {
  ...
  it("shows compact summaries without prices and resolves current truth only after open", async () => {
    ...
    await userEvent.click(screen.getByRole("button", { name: "Открыть комплект" }));
  ...
  it("preserves quantities, updates deliberately, and imports through one additive batch event", async () => {
    ...
    await userEvent.click(screen.getByRole("button", { name: "Сохранить количества" }));
    await waitFor(() => expect(mocks.update).toHaveBeenCalledWith({ listId: KIT, expectedRevision: 1, items: [{ itemId: ITEM, quantity: 4 }] }));
    await userEvent.click(screen.getByRole("button", { name: "Добавить доступные товары в подборку" }));
  ...
  it("supports save-as-new and Romanian copy", async () => {
    ...
    await userEvent.click(screen.getByRole("button", { name: "Deschide setul" }));
    ...
    await userEvent.click(screen.getByRole("button", { name: "Salvează setul" }));
  ...
  it("renders the guided empty state", () => {
    ...
    expect(screen.getByText(/Сохраните текущую подборку/)).toBeInTheDocument();
  });
});
