    ...
    <form
      ...
      onSubmit={(event) => {
        ...
        startTransition(async () => {
          const result = await createPurchasingListAction({
            name: String(form.get("name")),
            description: String(form.get("description")),
            visibility: String(form.get("visibility")) as "private" | "company",
          });
          if (result.success)
            router.push(`/cabinet/purchasing-lists/${result.data.id}`);
          else setError(copy.createListError);
        });
      }}
    >
      ...
      <button
        className="rounded-md bg-emerald-700 px-4 py-2 text-sm font-semibold text-white disabled:bg-zinc-300"
        disabled={pending}
        type="submit"
      >
        {pending ? copy.creating : copy.createList}
      </button>
    </form>
  );
}
