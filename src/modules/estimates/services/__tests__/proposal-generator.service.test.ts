...
describe("ProposalGeneratorService", () => {
  ...
  it("resolves catalog and external identities in bounded batches and initializes RETAIL selling price", async () => {
    ...
    const repository = { createEstimate, resolveExternalNomenclature, resolveServices };
    const companyAccess = {
    ...
    const result = await service.createEstimate(uuid("1"), {
      ...
      requirements: [
        ...
        { id: "two", sectionKey: "installation_works", description: "Монтаж", quantity: 2, unit: "pcs", resolution: "service", resolvedId: uuid("4"), resolvedLabel: "Монтаж видеокамеры" },
    ...
    expect(submitted.lines[1]).toMatchObject({ lineType: "service", serviceId: uuid("4"), sellingUnitPrice: null, unit: "pcs" });
