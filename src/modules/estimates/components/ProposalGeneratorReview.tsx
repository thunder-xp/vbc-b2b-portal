...
export function ProposalGeneratorReview({ requirements, currencyCode, onChange }: { requirements: GeneratorRequirement[]; currencyCode: string; onChange: (requirements: GeneratorRequirement[]) => void }) {
  ...
  return <div className="space-y-4">{GENERATOR_SECTIONS.map((section) => {
    ...
    return <section className="overflow-hidden rounded-md border border-zinc-200 bg-white" key={section.key}>
      ...
      <div className="divide-y divide-zinc-100">{lines.length ? lines.map((line) => <GeneratorLine currencyCode={currencyCode} key={line.id} line={line} onChange={(value) => patch(line.id, value)} onRemove={() => onChange(requirements.filter((item) => item.id !== line.id))} />) : <p className="px-4 py-4 text-sm text-zinc-500">Позиции не предложены.</p>}</div>
...
function GeneratorLine({ line, currencyCode, onChange, onRemove }: { line: GeneratorRequirement; currencyCode: string; onChange: (patch: Partial<GeneratorRequirement>) => void; onRemove: () => void }) {
  ...
  const select = (resolution: GeneratorResolutionKind, id: string, label: string) => {
    onChange({ resolution, resolvedId: id, resolvedLabel: label, description: label, resolvedSku: null, resolvedImageUrl: null, sellingUnitPrice: null, sellingCurrencyCode: null, sellingVatMode: null });
    setSearchOpen(false);
  };
  const selectCatalog = (item: CatalogResult) => {
    onChange({
      ...
      resolvedSku: item.sku, resolvedImageUrl: item.imageUrl,
  ...
  return <div className="grid min-w-0 gap-3 p-4 lg:grid-cols-[minmax(0,1fr)_8rem_13rem_auto] lg:items-start">
    <div className="min-w-0">{line.resolution === "unresolved" ? <><input aria-label="Описание позиции" className={inputClass} maxLength={500} onChange={(event) => onChange({ description: event.target.value })} value={line.description} /><p className="mt-1 text-xs text-amber-700">Требуется выбор позиции · Цена не указана</p></> : <div className="flex min-w-0 items-start gap-3">
      ...
      <div className="min-w-0"><p className="break-words text-sm font-semibold text-zinc-950">{line.resolvedLabel ?? line.description}</p>{line.resolvedSku && <p className="mt-0.5 text-xs text-zinc-500">SKU {line.resolvedSku}</p>}<p className="mt-1 text-xs text-emerald-700">Точное соответствие выбрано</p>{line.resolvedStockLabel && <p className="mt-1 text-xs font-medium text-zinc-700">{line.resolvedStockLabel}</p>}{line.requirementDescription && line.requirementDescription !== line.description && <p className="mt-1 text-xs text-zinc-500">Исходная потребность: {line.requirementDescription}</p>}</div>
    </div>}{line.sellingUnitPrice != null && <p className="mt-1 text-xs font-semibold">{line.sellingUnitPrice.toFixed(2)} {line.sellingCurrencyCode}{line.sellingVatMode === "included" ? " · НДС включён" : ""} · итого {(line.sellingUnitPrice * line.quantity).toFixed(2)} {line.sellingCurrencyCode}</p>}{line.assumption && <p className="mt-1 text-xs text-zinc-500">{line.assumption}</p>}</div>
    ...
    {searchOpen && <div className="space-y-3 rounded-md border border-zinc-200 bg-zinc-50 p-3 lg:col-span-4"><div className="flex flex-col gap-2 sm:flex-row"><input aria-label="Поиск соответствия" className={inputClass} onChange={(event) => setQuery(event.target.value)} value={query} /><button className={actionClassName.secondary} disabled={pending || query.trim().length < 2} onClick={() => search("own")} type="button">Найти</button><button className={actionClassName.secondary} disabled={pending || query.trim().length < 2} onClick={() => search("shared")} type="button">Расширить поиск</button></div>
      <div className="grid gap-2 md:grid-cols-2">{catalog.map((item) => <button className="min-h-11 rounded-md border bg-white p-3 text-left text-sm" key={item.id} onClick={() => selectCatalog(item)} type="button"><strong>{item.sku}</strong> · {item.name}<span className="block text-xs text-zinc-500">{item.retailPrice ?? "Цена уточняется"}</span></button>)}{external.map((item) => <button className="min-h-11 rounded-md border bg-white p-3 text-left text-sm" key={item.id} onClick={() => select(scope === "shared" ? "shared_nomenclature" : "own_nomenclature", item.id, item.name)} type="button">{item.name}<span className="block text-xs text-zinc-500">Цена не указана</span></button>)}</div>
      <div className="flex gap-2"><button className="min-h-11 text-sm font-semibold text-emerald-800" onClick={() => { const original = line.requirementDescription ?? line.description; onChange({ description: original, resolution: "unresolved", resolvedId: null, resolvedLabel: null, resolvedSku: null, resolvedImageUrl: null, sellingUnitPrice: null, sellingCurrencyCode: null, sellingVatMode: null }); setSearchOpen(false); }} type="button">Оставить как потребность</button><button aria-label="Закрыть поиск" className="ml-auto grid size-11 place-items-center" onClick={() => setSearchOpen(false)} type="button"><X className="size-4" /></button></div>
    </div>}
  </div>;
}
