...
import { conciseProposalDescription, proposalLineNumber, proposalVatLabels, sectionSubtotalLabel } from "../services/proposal-presentation";
    ...
    <div className="space-y-5 py-4">{proposal.sections.map((section) => {
      ...
      return <section className="break-inside-auto" key={section.name}>
        ...
        <div className="overflow-hidden"><table className="block w-full border-collapse text-xs sm:table sm:table-fixed print:table">
          ...
          <tbody className="block sm:table-row-group print:table-row-group">{section.lines.map((line, lineIndex) => <tr className="grid grid-cols-2 gap-x-3 border-b border-zinc-200 py-2 align-top sm:table-row sm:py-0 print:table-row" key={`${section.name}-${line.position}`}>
