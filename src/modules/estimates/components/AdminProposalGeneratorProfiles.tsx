...
type Target = { targetType: "catalog" | "external_nomenclature"; id: string; label: string; secondary: string | null };
...
export function AdminProposalGeneratorProfiles({ initialProfiles }: { initialProfiles: GeneratorProfileAdminRow[] }) {
  ...
  const update = (targetType: Target["targetType"] | "unresolved", targetId: string | null, label: string | null) => {
    ...
    startTransition(async () => {
      ...
      if (result.success) setProfiles((current) => current.map((profile) => profile.profileKey === selected.profileKey ? { ...profile, version: result.data.version, resolution: targetType === "catalog" ? "catalog" : targetType === "external_nomenclature" ? "shared_nomenclature" : "unresolved", resolvedId: targetId, resolvedLabel: label } : profile));
    });
  };
