...
export function EstimateBulkToolbar({ selectedCount, sections, dirty, disabled, showConfidentialControls = true, onMarkup, onDiscount, onMove, onQuantity, onResetPrice, onRemove, onClear }: {
  ...
  return <section aria-label="Действия с выбранными позициями" className="sticky top-[4.5rem] z-10 border-y border-emerald-200 bg-emerald-50 p-3 shadow-sm">
    <div className="flex flex-wrap items-center gap-2">
      ...
      {showConfidentialControls ? <><label className="flex items-center gap-1 text-xs">Наценка %<input aria-label="Наценка для выбранных" className={`${inputClass} w-20`} min="0" onChange={(event) => setMarkup(Number(event.target.value))} step="0.01" type="number" value={markup} /></label>
      <button aria-label="Применить наценку" className={buttonClass} disabled={disabled} onClick={() => onMarkup(markup)} type="button"><BadgePercent className="size-4" />Применить</button></> : null}
    ...
    <div className="mt-2 flex flex-wrap items-center gap-2">
      ...
      {showConfidentialControls ? <button className={buttonClass} disabled={disabled} onClick={onResetPrice} type="button"><PackageCheck className="size-4" />Текущая партнёрская цена</button> : null}
