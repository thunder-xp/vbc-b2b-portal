...
export function PublicRetailProductCard({ product, locale, badge, badgeCode, catalogState, filterableFacetKeys, showFacetShortcuts = false }: { product: PublicRetailProductSummaryDto; locale: PublicRetailLocale; badge?: string; badgeCode?: MerchandisingLabelCode; catalogState?: PublicRetailCatalogState; filterableFacetKeys?: ReadonlySet<string>; showFacetShortcuts?: boolean }) {
  ...
  return <CatalogProductCardFrame
    actions={<div className="grid grid-cols-[minmax(0,1fr)_2.75rem] items-start gap-2"><PublicRetailAddToCartButton compact locale={locale} publicProductId={product.id} source="catalog" /><Link aria-label={locale === "ro" ? `Mai multe detalii despre produsul ${product.name}` : `Подробнее о товаре ${product.name}`} className="grid size-11 place-items-center border border-zinc-300 transition-colors hover:border-[#3083EB] hover:text-[#236FD0] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#625DDD]" href={`/products/${product.slug}?lang=${locale}`} title={copy.details}><ArrowRight aria-hidden="true" className="size-4" /></Link></div>}
    ...
    context={showFacetShortcuts && facetShortcuts.length ? <ul aria-label={copy.specifications} className="flex h-5 min-w-0 gap-1 overflow-hidden">{facetShortcuts.map((item) => <li className="min-w-0" key={item.key}><Link aria-label={`${item.label}: ${item.value}`} className="block max-w-28 truncate border border-zinc-200 px-1.5 text-[11px] leading-[18px] text-zinc-600 transition-colors hover:border-[#3083EB] hover:text-[#236FD0] focus-visible:outline-2 focus-visible:outline-[#625DDD]" href={publicRetailFilterHref(locale, facetState, { facet: { key: item.key, value: item.value }, facetMode: "include" })} prefetch={false} title={`${item.label}: ${item.value}`}>{item.value}</Link></li>)}</ul> : null}
    density="compact"
    media={<Link className="group relative block aspect-[4/3] w-full min-w-0 max-w-full overflow-hidden bg-zinc-50 outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#625DDD]" href={`/products/${product.slug}?lang=${locale}`}>
    ...
    square
    title={<Link aria-label={product.name} className="line-clamp-2 h-10 rounded-sm text-sm font-semibold leading-5 text-zinc-950 outline-none transition-colors hover:text-[#236FD0] focus-visible:ring-2 focus-visible:ring-[#625DDD]" href={`/products/${product.slug}?lang=${locale}`} title={product.name}>{product.name}</Link>}
  />;
}
