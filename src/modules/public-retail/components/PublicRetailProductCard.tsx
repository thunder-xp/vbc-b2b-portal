...
export function PublicRetailProductCard({ product, locale }: { product: PublicRetailProductSummaryDto; locale: PublicRetailLocale }) {
  ...
  return <article className="group grid min-w-0 grid-cols-[minmax(0,1fr)] grid-rows-[auto_auto_1fr_auto_auto] overflow-hidden border border-zinc-200 bg-white">
    <Link className="relative block h-36 w-full min-w-0 max-w-full overflow-hidden bg-zinc-50 sm:h-44 xl:h-48 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald-700" href={`/products/${product.slug}?lang=${locale}`}>
      {product.image ? <Image alt={product.image.alt || product.name} className="size-full max-h-full max-w-full object-contain p-4 transition-transform duration-200 group-hover:scale-[1.02]" fill loading="lazy" sizes="(max-width: 640px) 50vw, (max-width: 1024px) 33vw, 25vw" src={product.image.url} /> : <span className="grid size-full max-h-full max-w-full place-items-center overflow-hidden text-zinc-300"><ImageIcon aria-hidden="true" className="size-12" /><span className="sr-only">{locale === "ro" ? "Imagine indisponibilă" : "Изображение отсутствует"}</span></span>}
    </Link>
