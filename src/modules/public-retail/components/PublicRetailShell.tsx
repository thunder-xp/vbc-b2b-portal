  ...
  return <div className="min-h-screen bg-white text-zinc-950" lang={locale}>
    <header className="sticky top-0 z-50 border-b border-zinc-200 bg-white/95 backdrop-blur">
      <div className="mx-auto flex min-h-17 max-w-[1440px] items-center gap-3 px-4 sm:px-6 lg:px-8">
        <Link aria-label="Novotech Systems Distribution" className="flex min-h-11 shrink-0 items-center gap-2.5" href={`/?lang=${locale}`}>
          <Image alt="" aria-hidden="true" className="size-11 shrink-0 object-contain" height={44} priority src="/retail/novotech-logo.svg" width={44} />
          <span className="min-w-0 leading-none">
            <span className="block text-sm font-bold tracking-[0.08em] sm:text-base">SYSTEMS</span>
            <span className="mt-1 block text-[10px] font-semibold leading-3 tracking-[0.16em] text-zinc-500">DISTRIBUTION</span>
          </span>
        </Link>
    ...
    <footer className="border-t border-zinc-800 bg-zinc-950 text-zinc-400" id="support">
      <div className="mx-auto grid max-w-[1440px] gap-x-8 gap-y-7 px-4 py-8 sm:grid-cols-2 sm:px-6 lg:grid-cols-[1.2fr_.8fr_.8fr_1.4fr] lg:px-8">
        <div><div className="flex items-center gap-3"><Image alt="" aria-hidden="true" className="size-10 object-contain" height={40} src="/retail/novotech-logo.svg" width={40} /><p className="text-sm font-semibold text-white">SYSTEMS <span className="ml-1 text-xs font-medium tracking-[0.12em] text-zinc-400">DISTRIBUTION</span></p></div><p className="mt-3 max-w-sm text-xs leading-5">{publicCompanyContent.descriptor[locale]}</p></div>
...
function FooterLink({ children, href }: { children: ReactNode; href: string }) {
  return <Link className="w-fit hover:text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#625DDD]" href={href}>{children}</Link>;
}
