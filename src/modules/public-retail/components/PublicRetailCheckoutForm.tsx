...
export function PublicRetailCheckoutForm({ checkout, locale, initialCustomer }: { checkout: PublicRetailCheckoutDto; locale: PublicRetailLocale; initialCustomer?: { name: string; phone: string; email: string; lockedIdentity: boolean } }) {
  ...
  return <form action={submit} className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-start">
    <div className="space-y-6">
      ...
      <label className="flex items-start gap-3 border border-blue-200 bg-blue-50 p-4 text-sm leading-6"><input className="mt-0.5 size-5 shrink-0 accent-blue-700" name="legalAccepted" required type="checkbox" /><span>{ru ? <>Я принимаю <Link className="font-semibold text-blue-800 underline underline-offset-4" href="/terms?lang=ru" target="_blank">Условия и положения</Link> и <Link className="font-semibold text-blue-800 underline underline-offset-4" href="/privacy?lang=ru" target="_blank">Политику конфиденциальности</Link>.</> : <>Accept <Link className="font-semibold text-blue-800 underline underline-offset-4" href="/terms?lang=ro" target="_blank">Termenii și condițiile</Link> și <Link className="font-semibold text-blue-800 underline underline-offset-4" href="/privacy?lang=ro" target="_blank">Politica de confidențialitate</Link>.</>}</span></label>
