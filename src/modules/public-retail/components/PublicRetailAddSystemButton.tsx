...
export function PublicRetailAddSystemButton({ locale, items, installationIntent }: { locale: PublicRetailLocale; items: Array<{ publicProductId: string; quantity: number; commercialGroup: "equipment" | "materials" }>; installationIntent: Record<string, boolean> | null }) {
  ...
  return <div><button className="inline-flex min-h-12 w-full items-center justify-center gap-2 bg-emerald-700 px-4 text-sm font-semibold text-white disabled:opacity-60" disabled={pending} onClick={() => startTransition(async () => {
    const result = await addPublicRetailCctvSystemAction({ locale, items, installationIntent, requestId });
