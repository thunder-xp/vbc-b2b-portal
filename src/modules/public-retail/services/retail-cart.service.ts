...
function normalizeIntent(value: Record<string, boolean> | null) { if (!value) return null; const keys = ["cameraInstallation", "cableLaying", "commissioning", "remoteViewing", "aiScenarioProgramming"] as const; if (Object.keys(value).some((key) => !keys.includes(key as typeof keys[number])) || keys.some((key) => typeof value[key] !== "boolean")) throw new RetailCartInputError(); return Object.fromEntries(keys.map((key) => [key, value[key]])); }
function normalizeWorkScope(value: Array<{ kind: string; quantity: number; unitCode: "piece" | "meter" | "service" }>) { const kinds = ["camera_installation", "cable_laying", "commissioning", "remote_configuration", "ai_scenario_programming"]; if (!Array.isArray(value) || value.length > 20 || value.some((item) => !kinds.includes(item.kind) || !Number.isFinite(item.quantity) || item.quantity <= 0 || item.quantity > 20_000 || !(["piece", "meter", "service"] as const).includes(item.unitCode))) throw new RetailCartInputError(); return value.map((item) => ({ kind: item.kind, quantity: item.quantity, unitCode: item.unitCode })); }
