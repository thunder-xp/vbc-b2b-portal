...
export function publicRetailVisibleCategories(categories: PublicRetailCategoryDto[]): PublicRetailCategoryDto[] {
  const excludedRootSlugs = new Set(["project-equipment-a7bad0fc"]);
  const excluded = new Set(categories
    .filter((category) => excludedRootSlugs.has(category.slug))
...
export const retailCopy = {
  ru: {
    ...
    customerAccount: "Личный кабинет",
    ...
    replenishmentCollection: "Поступление",
    replenishmentBadge: "Поступление",
  ...
  ro: {
    ...
    replenishmentCollection: "Reaprovizionare",
