...
export class SupabaseRetailCartRepository implements RetailCartRepository {
  ...
  async addBundle(tokenHash: string, command: Parameters<RetailCartRepository["addBundle"]>[1]) { return parsePublicRetailCartMutation(await this.rpc("add_public_retail_cart_cctv_bundle", { p_token_hash: tokenHash, p_items: command.items.map((item) => ({ public_product_id: item.publicProductId, quantity: item.quantity, commercial_group: item.commercialGroup })), p_installation_intent: command.installationIntent, p_request_id: command.requestId, p_fingerprint: command.fingerprint })); }
