...
export class ServiceHistoryService {
  ...
  async getPartner(userId: string, id: string) { await this.companyId(userId); return this.repository.getPartner(uuid(id)); }
  ...
  getAdmin(id: string) { return this.repository.getAdmin(uuid(id)); }
  diagnostics() { return this.repository.diagnostics(); }
  private async companyId(userId: string) { const membership=(await this.access.getOwnMemberships(userId)).find((item)=>item.status===MembershipStatus.Active); if(!membership) throw new Error("Active company is unavailable."); return (await this.access.getActiveCompanyContext(userId,membership.companyId)).company.id; }
}
