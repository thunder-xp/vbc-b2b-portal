...
export function QrShare({ url, svg, fileName }: { url: string; svg: string; fileName: string }) {
  ...
  return <div className="space-y-4" id="referral-link"><div className="break-all border border-zinc-200 bg-zinc-50 p-3 font-mono text-xs">{url}</div><div className="grid gap-2 sm:grid-cols-2">
    <button className={primaryButton} onClick={copy} type="button">{copied ? <Check size={18}/> : <Copy size={18}/>} {copied ? "Скопировано" : "Копировать ссылку"}</button>
    <a className={secondaryButton} href={url} rel="noreferrer" target="_blank"><ExternalLink size={18}/>Добавить клиента</a>
    <button className={secondaryButton} onClick={download} type="button"><Download size={18}/>Скачать QR</button>
    <button className={secondaryButton} onClick={() => window.print()} type="button"><Printer size={18}/>Печать</button>
  </div></div>;
}
