...
export class SupabasePartnerSupportRepository implements PartnerSupportRepository {
  ...
  private async rpc<T>(name: string, args: Record<string, unknown>): Promise<T> { const { data, error } = await (await createClient()).rpc(name, args); if (error) { const log = error.code === "PT409" ? console.info : console.error; log({ event: error.code === "PT409" ? "partner_support_conflict" : "partner_support_rpc_failed", rpc: name, code: error.code }); throw new PartnerSupportRepositoryError(error.code); } return data as T; }
}
