...
import { SupabasePartnerSupportRepository } from "./supabase.repository";
import type { SupportAssignee, SupportDashboardItem, SupportDiagnostics, SupportTicketDetail, SupportTicketPage } from "./types";
import { storeSupportAttachment } from "./storage";
...
export async function listSupportAssigneesAction(): Promise<ActionResult<SupportAssignee[]>> { try { await requireAdminPermission("support.assign"); return success("Исполнители загружены.", await support().listAssignees()); } catch (error) { return failureFromError(error); } }
export async function getAdminSupportTicketAction(id: string): Promise<ActionResult<SupportTicketDetail | null>> { try { await requireAnyAdminPermission(["support.view_all", "support.view_assigned"]); return success("Заявка загружена.", await support().getAdmin(id)); } catch (error) { return failureFromError(error); } }
export async function transitionSupportTicketAction(_state: ActionResult<null>, formData: FormData): Promise<ActionResult<null>> { try { await requireAdminPermission("support.manage"); const id = text(formData, "ticketId"); await support().transition({ ticketId: id, expectedVersion: Number(text(formData, "expectedVersion")), status: text(formData, "status"), partnerReply: text(formData, "partnerReply"), internalNote: text(formData, "internalNote"), assigneeId: nullable(formData, "assigneeId"), category: nullable(formData, "category"), effectivePriority: nullable(formData, "effectivePriority"), priorityReason: text(formData, "priorityReason") }); revalidatePath(`/admin/support/${id}`); revalidatePath(`/cabinet/support/${id}`); revalidatePath("/cabinet"); return success("Заявка обновлена.", null); } catch (error) { return failureFromError(error); } }
export async function getSupportDashboardAction(): Promise<ActionResult<SupportDashboardItem[]>> { try { return success("Заявки загружены.", await support().dashboard(await getAuthenticatedUserId())); } catch (error) { return failureFromError(error); } }
export async function getSupportDiagnosticsAction(): Promise<ActionResult<SupportDiagnostics>> { try { await requireAdminPermission("support.diagnostics.view"); return success("Диагностика загружена.", await support().diagnostics()); } catch (error) { return failureFromError(error); } }
