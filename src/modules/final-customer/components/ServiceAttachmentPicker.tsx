...
export function ServiceAttachmentPicker({ name = "files", locale }: { name?: string; locale: "ru" | "ro" }) {
  ...
  return <div className="space-y-2">
    <label className="block text-sm font-medium text-zinc-800">{ro ? "Fotografii sau PDF" : "Фотографии или PDF"}
      <input ref={inputRef} accept="image/jpeg,image/png,image/webp,application/pdf" className="mt-1.5 min-h-11 w-full rounded-lg border border-zinc-300 bg-white p-2 text-sm outline-none focus:border-emerald-700 focus:ring-2 focus:ring-emerald-100" multiple name={name} onChange={(event) => update(Array.from(event.target.files ?? []).slice(0, 5).map((file) => ({ file, preview: file.type.startsWith("image/") ? URL.createObjectURL(file) : "" })))} type="file" />
    </label>
    ...
    {items.length ? <ul className="grid gap-2 sm:grid-cols-2">{items.map(({ file, preview }, index) => <li className="flex min-w-0 items-center gap-2 rounded-lg border border-zinc-200 bg-zinc-50 p-2" key={`${file.name}-${file.lastModified}`}>
      ...
      <button aria-label={ro ? `Elimină ${file.name}` : `Удалить ${file.name}`} className="grid min-h-11 min-w-11 place-items-center rounded-lg transition-colors hover:bg-zinc-200 focus-visible:outline-2 focus-visible:outline-emerald-700 motion-reduce:transition-none" onClick={() => { if (preview) URL.revokeObjectURL(preview); update(items.filter((_, itemIndex) => itemIndex !== index)); }} type="button"><X aria-hidden className="size-4" /></button>
    </li>)}</ul> : null}
  </div>;
}
