...
export interface FinalCustomerRepository {
  ...
  updateAdminServiceRequest(input: Readonly<{ requestId: string; expectedVersion: number; status: CustomerServiceRequestStatus | null; customerReply: string; internalNote: string; actorUserId: string }>): Promise<{ messageId: string | null }>;
