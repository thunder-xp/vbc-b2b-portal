...
import {
  submitReservationRequestAction,
...
export function ReservationDetail({
  ...
  request: ReservationRequestDetailDto;
}) {
  const locale = usePartnerLocale();
  const copy = projectCopy(locale);
  const draft = request.status === ReservationRequestStatus.Draft;
  const showPartnerPrice = request.lines.some((line) =>
    Object.prototype.hasOwnProperty.call(line, "partnerPrice"),
  );
  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-4 border-b border-zinc-200 pb-5 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <Link
            ...
            href="/cabinet/reservation-requests"
          >
      ...
      {request.managerComment ? (
        <section className="border-l-4 border-emerald-600 bg-emerald-50 p-5">
          <h2 className="text-sm font-semibold">{copy.managerResponse}</h2>
      ...
      {draft ? (
        <DraftMetadata request={request} />
      ) : (
        <section className="grid gap-4 border border-zinc-200 bg-white p-5 sm:grid-cols-2">
          <Info
            label={copy.preferredDate}
            value={
              request.requestedDeliveryDate
                ? formatPartnerDate(request.requestedDeliveryDate, locale)
                : copy.notSpecified
            }
          />
          <Info
            label={copy.partnerComment}
            value={request.partnerComment ?? copy.noComment}
          />
        </section>
      )}
              ...
              <tr key={line.id}>
                ...
                <td className="px-4 py-4">
                  {draft ? (
                    <QuantityEditor
                      itemId={line.id}
                      max={line.specificationQuantity}
...
function DraftMetadata({ request }: { request: ReservationRequestDetailDto }) {
  ...
  return (
    <form
      className="grid gap-4 border border-zinc-200 bg-white p-5 sm:grid-cols-2"
      onSubmit={(event) => {
        event.preventDefault();
        const data = new FormData(event.currentTarget);
        startTransition(async () => {
          const result = await updateReservationRequestAction(request.id, {
    ...
    >
      <label className="text-sm font-medium">
        ...
        <input
          ...
          defaultValue={request.requestedDeliveryDate ?? ""}
      ...
      <label className="text-sm font-medium">
        {copy.comment}
        <textarea
          className="mt-2 min-h-24 w-full rounded-md border border-zinc-300 px-3 py-2"
          defaultValue={request.partnerComment ?? ""}
          maxLength={2000}
          name="partnerComment"
        />
      </label>
      <div className="sm:col-span-2">
        ...
        {message ? (
          <span className="ml-3 text-sm text-zinc-500" role="status">
            {message}
          </span>
        ) : null}
      </div>
...
function QuantityEditor({
  ...
  return (
    <input
      aria-label={copy.requestedQuantity}
      className="h-10 w-24 rounded-md border border-zinc-300 px-2"
      defaultValue={value}
      disabled={pending}
      max={max}
      min={1}
      onBlur={(event) => {
        ...
        startTransition(async () => {
          const result = await updateReservationQuantityAction(
            requestId,
            itemId,
            quantity,
          );
...
function SubmitRequest({ requestId }: { requestId: string }) {
  ...
  return (
    <div>
      <button
        className="rounded-md bg-zinc-950 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50"
        disabled={pending}
        onClick={() =>
          startTransition(async () => {
            const result = await submitReservationRequestAction(requestId);
            setMessage(result.success ? copy.actionSaved : copy.submitError);
            if (result.success) router.refresh();
          })
        }
        type="button"
