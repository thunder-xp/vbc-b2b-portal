...
export function ReservationDetail({ request }: { request: ReservationRequestDetailDto }) {
  ...
  return <div className="space-y-6">
    ...
    <section className="overflow-x-auto border border-zinc-200 bg-white"><table className="w-full min-w-[900px] text-left text-sm"><thead className="bg-zinc-50 text-xs uppercase text-zinc-500"><tr><th className="px-4 py-3">Товар</th><th className="px-4 py-3">Запрошено</th><th className="px-4 py-3">Одобрено</th>{showPartnerPrice ? <th className="px-4 py-3">Партнёрская</th> : null}<th className="px-4 py-3">Розница</th><th className="px-4 py-3">Наличие</th><th className="px-4 py-3">Поступление</th></tr></thead><tbody className="divide-y divide-zinc-100">{request.lines.map((line) => <tr key={line.id}><td className="px-4 py-4"><Link className="font-semibold hover:text-emerald-700" href={`/cabinet/catalog/${line.slug}`}>{line.productName}</Link><div className="text-xs text-zinc-500">SKU {line.sku} · максимум {line.specificationQuantity}</div></td><td className="px-4 py-4">{draft ? <QuantityEditor itemId={line.id} max={line.specificationQuantity} requestId={request.id} value={line.requestedQuantity} /> : line.requestedQuantity}</td><td className="px-4 py-4">{line.approvedQuantity ?? "—"}</td>{showPartnerPrice ? <td className="px-4 py-4">{line.partnerPrice ?? "Недоступно"}</td> : null}<td className="px-4 py-4">{line.retailPrice ?? "Недоступно"}</td><td className="px-4 py-4">{line.availability.availableStock ?? "Уточняется"}</td><td className="px-4 py-4">{line.availability.nearestArrivalDate ?? "—"}{line.availability.nearestArrivalQuantity !== null ? ` · ${line.availability.nearestArrivalQuantity} шт.` : ""}</td></tr>)}</tbody></table></section>
