  ...
  return (
    <div className="space-y-5">
      ...
      <form action={action} className="grid gap-5 sm:grid-cols-2">
        ...
        <label className="text-sm font-medium">Тип цены из 1С<select name="priceProfileId" defaultValue={draft.selectedPriceProfileId ?? ""} className="mt-2 min-h-11 w-full rounded-md border border-zinc-300 bg-white px-3"><option value="">Не применять партнёрский тип цены (только розничные цены)</option>{candidate?.priceProfiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.name}{profile.code ? ` (${profile.code})` : ""}</option>)}</select></label>
