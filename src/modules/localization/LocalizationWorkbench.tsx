...
function LocalizationEditor({ canManage, entityType, item }: {
  ...
  const [localizedShortDescription, setLocalizedShortDescription] = useState(item.localizedShortDescription ?? "");
  ...
  const content = {
    localizedName, ...(entityType === "category"
      ? { intro: localizedDescription }
      : { shortDescription: localizedShortDescription, description: localizedDescription }),
  ...
  return <details className="group" open={item.effectiveStatus === "outdated"}>
    ...
    <div className="grid gap-5 border-t border-zinc-100 bg-zinc-50 p-4 lg:grid-cols-2">
      ...
      <section aria-label="Румынская локализация" className="space-y-3"><h3 className="text-xs font-semibold uppercase text-blue-700">Локализация · RO</h3>
        ...
        <p className="text-xs text-zinc-500">Источник: {contentSourceLabel(item.contentSource)} · Обновлено: {formatDate(item.translatedAt)} · Проверка: {formatDate(item.reviewedAt)}</p>
        ...
        {entityType === "product" ? <Field label="Краткое описание"><textarea className="min-h-20 w-full resize-y border border-zinc-300 bg-white px-3 py-2 text-sm" disabled={!canManage || pending} value={localizedShortDescription} onChange={(event)=>setLocalizedShortDescription(event.target.value)} /></Field> : null}
...
function StatusBadge({ status }: { status: LocalizationWorkbenchItem["effectiveStatus"] }) {
  const copy = { missing: "Нет перевода", draft: "Черновик", reviewed: "Проверено", outdated: "Устарело" }[status];
...
function contentSourceLabel(source: LocalizationWorkbenchItem["contentSource"]) {
  return source === "machine" ? "машинный черновик" : source === "imported" ? "импорт" : source === "manual" ? "вручную" : "—";
}
