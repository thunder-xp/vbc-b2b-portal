...
export const SERVICE_ATTACHMENT_MAX_COUNT = 5;
export const SERVICE_ATTACHMENT_MIME = ["image/jpeg", "image/png", "image/webp", "application/pdf"] as const;

export function hasValidServiceFileSignature(bytes: Uint8Array, mime: string): boolean {
  ...
  return mime === "image/webp" && starts(bytes, [0x52, 0x49, 0x46, 0x46])
    && bytes.length > 11 && String.fromCharCode(...bytes.slice(8, 12)) === "WEBP";
}
