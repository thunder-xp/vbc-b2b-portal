...
export function ExternalPriceComparison({ commercialView, locale, prices }: { commercialView?: ProductCommercialViewDto; locale: PartnerLocale; prices: CurrentExternalPriceDto[] }) {
  ...
  return <section aria-labelledby="external-price-heading" className="mt-6 border-t border-zinc-200 pt-5"><h2 className="text-base font-semibold" id="external-price-heading">{copy.competitorPrices}</h2><div className="mt-3 space-y-3">{groupPrices(prices).map(group=><article className="border border-zinc-200 bg-zinc-50 p-4" key={group.sourceId}><div className="flex flex-wrap items-baseline justify-between gap-2"><h3 className="font-semibold text-zinc-950">{group.sourceName}</h3><p className="text-xs text-zinc-500">{copy.priceFrom} {formatPartnerDate(group.observedAt,locale,{day:"2-digit",month:"2-digit",year:"numeric",timeZone:"UTC"})}</p></div><div className="mt-3 grid gap-3 sm:grid-cols-2">{group.prices.map(price=><ComparisonRow commercialView={commercialView} copy={copy} key={price.priceType} locale={locale} price={price}/>)}</div></article>)}</div></section>;
}
