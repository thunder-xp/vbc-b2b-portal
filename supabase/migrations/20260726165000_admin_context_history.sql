...
begin
  ...
  if (p_company_id is null) = (p_user_id is null) then
    raise exception 'Exactly one audit context is required.'
      using errcode = '22023';
  end if;
  ...
  with audit_rows as (
    ...
    where (p_company_id is not null and event.company_id = p_company_id)
    ...
    where (p_company_id is not null and request.company_id = p_company_id)
      or (p_user_id is not null and request.user_profile_id = p_user_id)
  ),
  counted as (
...
is 'Bounded context audit projection. Excludes invitation tokens, sessions, credentials, confidential prices, and raw payloads.';
