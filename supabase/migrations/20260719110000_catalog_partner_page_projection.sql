-- Keep the sort/count aggregate and visible-page commercial projection inside
-- one authenticated database call. The base function already validates active
-- company membership, catalog.view, input bounds, and allowlisted sort names.
alter function public.catalog_partner_page(uuid, uuid, uuid, text, text, jsonb, text, integer, integer)
  security definer;
...
begin
  ...
  with rates as (
    ...
    select item, item ->> 'id' as product_id, ordinal
    from jsonb_array_elements(base_result -> 'items') with ordinality page(item, ordinal)
  )
  select coalesce(jsonb_agg(
    page_items.item || jsonb_build_object(
      ...
      'msrp_price_currency', msrp_price.currency,
...
revoke all on function public.catalog_partner_page_v2(uuid, uuid, uuid, text, text, jsonb, text, integer, integer)
  from public, anon;
grant execute on function public.catalog_partner_page_v2(uuid, uuid, uuid, text, text, jsonb, text, integer, integer)
  to authenticated;

comment on function public.catalog_partner_page_v2(uuid, uuid, uuid, text, text, jsonb, text, integer, integer) is
  'Returns one bounded, company-authorized catalog page and its visible commercial card projection in one call.';
