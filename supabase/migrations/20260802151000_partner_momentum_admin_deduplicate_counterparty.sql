...
begin
  ...
  with allowed as (
    ...
    left join lateral (
      select directory.fiscal_code
      from public.one_c_counterparties directory
      where lower(directory.external_1c_id) = lower(company.external_1c_id)
      order by
        directory.is_published desc,
        directory.is_active desc,
        directory.is_deleted asc,
        directory.synchronized_at desc,
        directory.id desc
      limit 1
