...
begin
  if not exists (
    select 1
    from jsonb_array_elements(new.installation_intent_snapshot) bundle,
      lateral jsonb_each(
        case
          when jsonb_typeof(bundle->'intent') = 'object' then bundle->'intent'
          else '{}'::jsonb
        end
      ) intent
    where jsonb_typeof(intent.value) = 'boolean'
      and (intent.value)::boolean
  ) then
    return new;
  end if;
