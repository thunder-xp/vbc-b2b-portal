...
alter table public.purchasing_list_items
  ...
  add column if not exists product_image_url_snapshot text null,
  add column if not exists created_by uuid null references public.user_profiles(id) on delete set null;

alter table public.purchasing_list_items
  drop constraint if exists purchasing_list_items_product_name_snapshot_check;
alter table public.purchasing_list_items
  add constraint purchasing_list_items_product_name_snapshot_check
  check (product_name_snapshot is null or length(product_name_snapshot) between 1 and 500);

alter table public.purchasing_list_items
  drop constraint if exists purchasing_list_items_product_image_url_snapshot_check;
alter table public.purchasing_list_items
  add constraint purchasing_list_items_product_image_url_snapshot_check
  check (product_image_url_snapshot is null or length(product_image_url_snapshot) <= 2048);
...
begin
  ...
  order by list.is_system_favorites asc, list.created_at desc, list.id
...
declare
  ...
  product_snapshot public.catalog_products;
begin
  if auth.uid() is null or not public.has_permission(target_company_id, 'purchasing_lists.manage') then
    raise exception 'Favorite update denied.' using errcode = '42501';
  end if;

  select product.* into product_snapshot
  from public.catalog_products product
  where product.id = target_product_id and product.is_active and product.is_visible;
  if product_snapshot.id is null then
    raise exception 'Favorite update denied.' using errcode = '42501';
  end if;
  ...
  if target_saved then
    ...
    select candidate.id into target_list_id
    from public.purchasing_lists candidate
    where candidate.company_id = target_company_id
      and candidate.created_by = auth.uid()
      and candidate.is_system_favorites
    for update;

    select coalesce(max(item.position), 0) + 1 into next_position
    from public.purchasing_list_items item
    where item.list_id = target_list_id;

    insert into public.purchasing_list_items(
      list_id, product_id, quantity, position, source_type,
      product_name_snapshot, product_image_url_snapshot, created_by
    )
    values (
      target_list_id, target_product_id, 1, next_position, 'favorite',
      product_snapshot.name, coalesce(product_snapshot.image_source_url, product_snapshot.image_url), auth.uid()
    )
    on conflict on constraint purchasing_list_items_product_unique do nothing;
...
comment on column public.purchasing_list_items.product_name_snapshot is
  'Display fallback captured when an item is favorited; current catalog data remains authoritative.';
comment on column public.purchasing_list_items.product_image_url_snapshot is
  'Display fallback captured when an item is favorited; current catalog data remains authoritative.';
comment on column public.purchasing_list_items.created_by is
  'Actor who added the item when the mutation path records authorship; nullable for backward-compatible historical rows.';
