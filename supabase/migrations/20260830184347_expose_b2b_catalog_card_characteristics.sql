...
-- Keeps the commercial page aggregate authoritative while enriching only its
-- bounded result page with governed, filterable specification shortcuts.
create or replace function public.catalog_partner_page_v4(
  p_company_id uuid,
...
begin
  ...
  ranked_characteristics as (
    select
      ...
      row_number() over (
        ...
        order by
          case
            when attribute.label ~* '(micro.?sd|memory.?card|card.?capacity|РєР°СЂС‚Р°.?РїР°РјСЏС‚Рё|РѕР±СЉРµРј.?РїР°РјСЏС‚Рё)' then 0
            when attribute.label ~* '(analytics|ivs|smd|Р°РЅР°Р»РёС‚РёРє)' then 1
            when attribute.label ~* '(ingress.?protection|protection.?class|ip.?rating|СЃС‚РµРїРµРЅСЊ.?Р·Р°С‰РёС‚С‹|РєР»Р°СЃСЃ.?Р·Р°С‰РёС‚С‹)' then 2
            when attribute.label ~* '(resolution|Р°Р·СЂРµС€РµРЅРё)' then 3
            when attribute.label ~* '(technology|РµС…РЅРѕР»РѕРі)' then 4
            when attribute.label ~* '(microphone|РјРёРєСЂРѕС„РѕРЅ)' then 5
            when attribute.label ~* '(lens|focal|РѕР±СЉРµРєС‚РёРІ|С„РѕРєСѓСЃ)' then 6
            when attribute.label ~* '(form.?factor|С„РѕСЂРј.?С„Р°РєС‚РѕСЂ)' then 7
            else 8
          end,
          attribute.label,
          attribute.attribute_key
  ...
  characteristics_by_product as (
    ...
    where characteristic.characteristic_rank <= 3
...
revoke all on function public.catalog_partner_page_v4(
  uuid, uuid, uuid, text, text, jsonb, text, text, integer, integer
) from public, anon;
grant execute on function public.catalog_partner_page_v4(
  uuid, uuid, uuid, text, text, jsonb, text, text, integer, integer
) to authenticated;

comment on function public.catalog_partner_page_v4(
  uuid, uuid, uuid, text, text, jsonb, text, text, integer, integer
) is
  'Returns the canonical permission-aware partner catalog page plus at most three governed filterable characteristics for each bounded page item.';
