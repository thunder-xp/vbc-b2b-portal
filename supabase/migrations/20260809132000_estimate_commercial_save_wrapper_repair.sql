-- Preserve the hardened section-retention wrapper while expanding the composite
-- estimate returned by the implementation into the row variable correctly.
...
begin
  ...
  select * into result
  from public.save_estimate_commercial_draft_impl(
    target_estimate_id,
    expected_revision,
    estimate_settings,
    section_payload,
