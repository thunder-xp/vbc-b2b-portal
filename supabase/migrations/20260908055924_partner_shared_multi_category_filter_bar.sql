...
begin
  if p_category_id is not null and coalesce(cardinality(p_category_ids), 0) > 0
    or coalesce(cardinality(p_category_ids), 0) > 24
    or exists (
      select 1
...
comment on function public.catalog_partner_page_v6(
  uuid, uuid, uuid[], uuid, text, text, jsonb, text, text, integer, integer
) is 'Returns one bounded permission-aware partner catalog page for either one canonical category or an bounded governed root-category selection.';

comment on function public.catalog_partner_facets_v3(
  uuid, uuid, uuid[], uuid, text, text, jsonb, text, integer
) is 'Returns bounded partner catalog facets for either one canonical category or an bounded governed root-category selection.';

create or replace function public.get_partner_previously_purchased_products_v3(
  p_company_id uuid,
...
begin
  ...
  if p_limit not between 1 and 24
    ...
    or coalesce(cardinality(p_category_ids), 0) > 50
  then
    raise exception 'Invalid previously purchased products page.' using errcode = '22023';
  end if;

  select coalesce(array_agg(distinct category_id), '{}')
  into selected_category_ids
  from unnest(coalesce(p_category_ids, '{}')) as selected(category_id)
  join public.catalog_categories root_category
    on root_category.id = selected.category_id
   and root_category.parent_id is null
   and root_category.is_active;
  ...
  with recursive category_tree as (
    select category.id as root_id, category.id as category_id
    from public.catalog_categories category
    where category.parent_id is null and category.is_active
    union all
    select tree.root_id, child.id
    from category_tree tree
    join public.catalog_categories child on child.parent_id = tree.category_id
    where child.is_active
  ), completed_order_products as materialized (
    select
    ...
    select
      ...
      category.name as category_name,
      category.slug as category_slug,
      root_category.id as root_category_id,
      root_category.external_1c_id as root_category_external_1c_id,
      root_category.name as root_category_name,
      root_category.slug as root_category_slug,
    ...
    left join category_tree category_path on category_path.category_id = product.category_id
    left join public.catalog_categories root_category
      on root_category.id = category_path.root_id and root_category.is_active
  ), category_facets as (
    select root_category.id,
      root_category.external_1c_id,
      root_category.name,
      root_category.slug,
      count(product.id)::integer as product_count
    from public.catalog_categories root_category
    left join historical_products product on product.root_category_id = root_category.id
    where root_category.parent_id is null
      and root_category.is_active
      and root_category.external_1c_id is not null
    group by root_category.id, root_category.external_1c_id,
      root_category.name, root_category.slug
    ...
    where (
      ...
      and (
        ...
        or product.root_category_id = any(selected_category_ids)
      )
  ), commercial as (
    select
      product.*,
  ...
  select jsonb_build_object(
    ...
    'categories', coalesce((select jsonb_agg(jsonb_build_object(
      'id', category.id, 'external1cId', category.external_1c_id,
      'parentId', null, 'name', category.name, 'slug', category.slug,
      'productCount', category.product_count
    ) order by category.name, category.id) from category_facets category), '[]'::jsonb),
    'allCount', (select count(*) from historical_products),
    'totalCount', (select count(*) from commercial)
  ) into result;
...
revoke all on function public.get_partner_previously_purchased_products_v3(uuid, uuid[], text, integer, integer)
  from public, anon;
grant execute on function public.get_partner_previously_purchased_products_v3(uuid, uuid[], text, integer, integer)
  to authenticated;

comment on function public.get_partner_previously_purchased_products_v3(uuid, uuid[], text, integer, integer) is
  'Returns one bounded company-authorized page of distinct completed-history products with governed root-category OR filtering, root counts, and current commercial projections.';
