...
begin
  ...
  if target_version.id is null
    ...
    or target_estimate.revision <> expected_estimate_revision
    or target_version.estimate_revision <> expected_estimate_revision then
    raise exception 'Accepted Estimate version changed before transfer.' using errcode = '40001';
  end if;
  ...
  if input_count <> expected_count
    ...
    raise exception 'Estimate transfer lines do not match the accepted version.' using errcode = '40001';
  end if;
