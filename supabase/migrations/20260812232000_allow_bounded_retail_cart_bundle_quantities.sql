alter table public.retail_cart_items
  drop constraint retail_cart_items_quantity_check,
  add constraint retail_cart_items_quantity_check check (quantity between 1 and 20000);
...
begin
  ...
  select count(*) into requested_count from requested where quantity between 1 and 20000 and commercial_group in ('equipment','materials');
...
begin
  if p_quantity < 1 or p_quantity > (case when p_bundle_id is null then 99 else 20000 end) then raise exception 'Invalid quantity.' using errcode='22023'; end if;
...
revoke all on function public.add_public_retail_cart_cctv_bundle(text,jsonb,jsonb,uuid,text), public.update_public_retail_cart_quantity(text,uuid,uuid,integer,bigint) from public, authenticated;
grant execute on function public.add_public_retail_cart_cctv_bundle(text,jsonb,jsonb,uuid,text), public.update_public_retail_cart_quantity(text,uuid,uuid,integer,bigint) to anon, service_role;
