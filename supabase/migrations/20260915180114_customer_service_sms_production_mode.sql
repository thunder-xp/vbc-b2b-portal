...
begin
  ...
  if jsonb_typeof(p_intent) <> 'object' or jsonb_typeof(p_delivery) <> 'object'
    ...
    or p_delivery->>'channelMode' not in ('DISABLED','SANDBOX','LIVE')
    or p_delivery->>'requestedMode' not in ('DISABLED','SANDBOX','LIVE')
...
comment on function public.persist_customer_service_sms_intent(jsonb,jsonb) is
  'Service-only Final Customer adapter into the common durable Omnichannel outbox. It accepts governed SANDBOX/LIVE delivery modes and canonicalizes the verified Supabase Auth phone before identity comparison.';
