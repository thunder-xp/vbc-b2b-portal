...
begin
  ...
  if p_entity_type='category' then
    ...
    with limited as (
      select 'product'::text entity_type,ranked.product_id entity_id,ranked.sku entity_reference,
        ranked.sku,p_locale locale,ranked.source_name,ranked.current_hash,
        null::text localized_name,null::text short_description,null::text description,
        null::text seo_title,null::text seo_description,'missing'::text status,ranked.selection_order
      from public.rank_portal_localization_product_batch(p_locale,p_limit) ranked
    )
    select coalesce(jsonb_agg(to_jsonb(limited)-'selection_order' order by selection_order),'[]'::jsonb)
...
revoke all on function public.export_portal_localization_rows(text,text,text,integer) from public, anon, authenticated;
grant execute on function public.rank_portal_localization_product_batch(text,integer) to service_role;
grant execute on function public.export_portal_localization_rows(text,text,text,integer) to service_role;
