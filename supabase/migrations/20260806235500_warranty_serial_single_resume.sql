...
set safe_error_code='superseded_full_bootstrap',
...
begin
  ...
  if not found then
    ...
    where status='failed'
      ...
      and safe_error_code is distinct from 'superseded_full_bootstrap'
    order by pages_fetched desc,started_at desc limit 1 for update;
    if found then
