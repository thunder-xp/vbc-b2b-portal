...
alter table public.partner_product_cobuy_associations
  ...
  drop constraint partner_product_cobuy_metrics_check,
  drop constraint partner_product_cobuy_window_check,
  drop column window_start,
  drop column window_end;

alter table public.partner_product_cobuy_state
  drop constraint partner_product_cobuy_state_window_check,
  drop constraint partner_product_cobuy_state_counts_check,
  drop column window_start,
  drop column window_end,
  add column history_mode text not null
    default 'all_time_authoritative_history';

drop index if exists public.partner_order_history_completed_cobuy_window_idx;
...
begin
  ...
  with governed_order_products as materialized (
    ...
    select measured.*
    from measured
    where measured.pair_order_count >= 5
...
alter table public.partner_product_cobuy_associations
  ...
  add constraint partner_product_cobuy_metrics_check check (
    confidence >= 0.10 and confidence <= 1 and lift > 1
  );

alter table public.partner_product_cobuy_state
  add constraint partner_product_cobuy_state_history_mode_check check (
    history_mode = 'all_time_authoritative_history'
  ),
  add constraint partner_product_cobuy_state_counts_check check (
    total_order_count >= 0
    and eligible_source_product_count >= 0
    and association_count >= 0
    and minimum_pair_order_count >= 5
...
comment on table public.partner_product_cobuy_associations is
  'Private all-time anonymous B2B order-level co-purchase projection. Metrics are server-only and never partner-visible.';
comment on table public.partner_product_cobuy_state is
  'Singleton all-time refresh evidence and calibrated thresholds for the private co-purchase projection.';
comment on function public.refresh_partner_product_cobuy_associations() is
  'Rebuilds one canonical all-time directional co-purchase projection from governed global B2B history using source counterparty identity.';
