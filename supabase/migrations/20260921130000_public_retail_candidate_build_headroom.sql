-- Bound candidate memory and compute category localization validity once per category.
-- This removes checksum sort spill and repeated category-hash scans from every product path.
...
begin
  ...
  drop table if exists pg_temp.retail_candidate_products;
  drop table if exists pg_temp.retail_candidate_categories;
  drop table if exists pg_temp.retail_candidate_facets;
  drop table if exists pg_temp.retail_candidate_category_localizations;
  ...
  create temporary table retail_candidate_category_localizations (
  ...
  insert into pg_temp.retail_candidate_category_localizations (
    public_id, localized_name, intro, seo_title, seo_description
  )
  select identity.public_id, localization.localized_name, localization.intro,
    localization.seo_title, localization.seo_description
  from public.public_retail_category_identities identity
  join public.category_localizations localization
    on localization.category_id = identity.source_category_id
   and localization.locale = 'ro'
  ...
  update pg_temp.retail_candidate_products snapshot set
    ...
    category_path=(select coalesce(jsonb_agg(path.value || jsonb_build_object('nameRo',coalesce(category_localization.localized_name,path.value->>'nameRu')) order by path.ordinal),'[]'::jsonb)
      ...
      left join pg_temp.retail_candidate_category_localizations category_localization
        on category_localization.public_id=(path.value->>'id')::uuid),
  ...
  from pg_temp.retail_candidate_category_localizations localization
  where snapshot.publication_id=p_publication_id
    and snapshot.public_id=localization.public_id;
