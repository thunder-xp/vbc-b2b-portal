-- Integrates the current warehouse replenishment into the canonical partner catalog query.
-- The singleton read model remains authoritative; no merchandising assignment is synthesized.
...
create or replace function public.catalog_partner_facets_v2(
  p_company_id uuid,
  p_category_id uuid default null,
  p_brand_id uuid default null,
  p_search text default null,
...
comment on function public.catalog_partner_page_v3(
  ...
  'Returns one permission-aware catalog page for a governed merchandising or current replenishment selection before sorting and pagination.';
...
comment on function public.catalog_partner_facets_v2(
  ...
  'Returns bounded attribute facets scoped to the governed partner catalog selection in one query.';
