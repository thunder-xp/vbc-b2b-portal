...
create function public.get_partner_previously_purchased_products_v3(
  p_company_id uuid,
...
begin
  ...
  with completed_order_products as materialized (
    ...
    select product.*
    from historical_products product
    where cardinality(selected_category_ids) = 0
      or product.category_id = any(selected_category_ids)
  ), commercial as (
    select
      product.*,
...
comment on function public.get_partner_previously_purchased_products_v3(uuid, uuid[], text, integer, integer) is
  'Returns one bounded company-authorized page of distinct completed-history products with multi-category OR filtering, category facets, and current governed commercial projections.';
