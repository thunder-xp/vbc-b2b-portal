-- A semantic replay of the active manual rate is a read-only success. Material
-- changes continue to create a new immutable version and audit event.
...
begin
  ...
  if current_rate.id is not null and p_effective_at::date < current_rate.effective_at::date then
  ...
  if current_rate.id is not null
    and current_rate.rate = p_rate::numeric(18, 6)
    and current_rate.effective_at::date = p_effective_at::date
    and current_rate.source_note = normalized_source_note
    and current_rate.evidence_comment is not distinct from normalized_evidence_comment
  then
...
comment on function public.publish_manual_commercial_exchange_rate_v2(text, numeric, timestamptz, text, text) is
  'Publishes immutable purpose-based rates; semantic replay returns the active version without a new row or audit event.';
