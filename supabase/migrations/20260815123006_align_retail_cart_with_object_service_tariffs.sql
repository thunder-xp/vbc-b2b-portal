-- Keep cart validation on the same object-specific tariff resolver used by the calculator.
...
begin
  ...
  if scope_count=0 then
    ...
    select public.resolve_cctv_object_services(
      target_object_type,
      array(select kind from jsonb_to_recordset(p_work_scope) x(kind text,quantity numeric,unit_code text))
    ) into resolved_services;

    with scope as (
      select kind,quantity,unit_code,ordinality
      from jsonb_to_recordset(p_work_scope) with ordinality x(kind text,quantity numeric,unit_code text,ordinality bigint)
    ), resolved as (
