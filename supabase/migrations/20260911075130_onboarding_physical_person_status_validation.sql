...
declare
  ...
  valid_price_profile_count integer;
  canonical_price_profile_id uuid;
  effective_price_profile_id uuid;
  selected_price_profile_valid boolean;
begin
  ...
  if p_step = 1 then
    ...
    select count(*), max(price_profile.id::text)::uuid
      into valid_price_profile_count, canonical_price_profile_id
    ...
    select exists (
      ...
      where profile_counterparty.id = draft.confirmed_counterparty_id
        ...
        and price_profile.is_published and price_profile.is_active
        and not price_profile.is_deleted
    ) into selected_price_profile_valid;

    effective_price_profile_id := p_price_profile_id;
    if (p_price_profile_id is null or not selected_price_profile_valid)
      and valid_price_profile_count = 1 then
      effective_price_profile_id := canonical_price_profile_id;
    elsif p_price_profile_id is not null and not selected_price_profile_valid then
      if valid_price_profile_count = 0 then
        raise exception 'onboarding_price_type_required' using errcode = '22023';
      end if;
      raise exception 'onboarding_partner_status_selection_required' using errcode = '22023';
    end if;
    update public.onboarding_approval_drafts
      set assigned_internal_manager_user_id = p_assigned_manager_id,
          selected_price_profile_id = effective_price_profile_id,
    ...
    if p_initial_profile <> 'retail_only' then
      select exists (
        select 1
        from public.one_c_counterparties profile_counterparty
        join public.one_c_counterparty_price_profiles price_profile
          on price_profile.counterparty_external_1c_id = profile_counterparty.external_1c_id
        where profile_counterparty.id = draft.confirmed_counterparty_id
          and price_profile.id = draft.selected_price_profile_id
          and price_profile.is_published and price_profile.is_active
          and not price_profile.is_deleted
      ) into selected_price_profile_valid;

      if not selected_price_profile_valid then
        select count(*), max(price_profile.id::text)::uuid
          into valid_price_profile_count, canonical_price_profile_id
        from public.one_c_counterparties profile_counterparty
        join public.one_c_counterparty_price_profiles price_profile
        ...
        if valid_price_profile_count = 1 then
          draft.selected_price_profile_id := canonical_price_profile_id;
        elsif valid_price_profile_count = 0 then
          raise exception 'onboarding_price_type_required' using errcode = '22023';
        else
          raise exception 'onboarding_partner_status_selection_required' using errcode = '22023';
        end if;
    ...
    update public.onboarding_approval_drafts
      set selected_price_profile_id = draft.selected_price_profile_id,
          initial_business_profile = p_initial_profile,
          finance_access = case
...
comment on function public.save_onboarding_approval_draft(
  uuid, integer, integer, smallint, uuid, uuid, uuid, text, text, boolean, boolean
) is 'Persists one versioned onboarding wizard step, auto-selecting the sole governed 1C price profile when a stale or empty selection would otherwise block access setup.';
