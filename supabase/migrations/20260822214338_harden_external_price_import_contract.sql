...
alter table public.external_price_uploads
  add constraint external_price_uploads_hash_check
    check (source_file_hash ~ '^[0-9a-f]{64}$'),
  add constraint external_price_uploads_storage_key_check
    check (
      storage_key = partner_company_id::text || '/' || id::text || '/' ||
        source_file_hash || '.' || file_format
    );
...
begin
  if not public.can_access_external_prices(
    p_company_id,
    'external_prices.manage'
  ) then
  ...
  if exists (
    ...
    raise exception 'Review all ambiguous rows before applying.'
      using errcode = 'PT409';
  end if;
  if not exists (
    select 1
    ...
    raise exception 'At least one governed product match is required.'
      using errcode = 'PT409';
  end if;
  ...
  select observation.partner_company_id,
    observation.external_price_source_id, observation.catalog_product_id,
    price.price_type, observation.id, price.amount, observation.currency,
    observation.observed_at, observation.upload_id
  from public.external_price_observations observation
  cross join lateral (
    values
      ('partner'::text, observation.partner_price),
      ('retail'::text, observation.retail_price)
  ...
  insert into public.external_price_events(
    upload_id, partner_company_id, actor_user_id, event_type,
    safe_metadata
  ) values (
