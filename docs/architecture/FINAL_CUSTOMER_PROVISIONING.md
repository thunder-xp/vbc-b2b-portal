...
The implementation now has a production-proven Final Customer Counterparty match/read-back contract, canonical minimal DTO, and governed idempotency/reconciliation semantics. Runtime processing remains gated by `ONE_C_CUSTOMER_PROVISIONING_ENABLED=true`. Creation is deliberately non-executable until Finance/1C supplies the missing contact-kind and linked-person create contract; `ONE_C_CUSTOMER_WRITE_READY` cannot override that safeguard. Jobs remain pending and active local accounts are unaffected while the gate is disabled.
...
The 2026-09-20 production `$metadata` audit corrected the contact contract: `Catalog_Контрагенты` does **not** expose scalar `Телефон` or `ЭлектроннаяПочта` properties. It exposes bounded lookup fields `НомерТелефонаДляПоиска` and `АдресЭПДляПоиска`, plus the `КонтактнаяИнформация` collection whose rows carry `Тип`, `Вид_Key`, `Представление`, `АдресЭП`, `НомерТелефона`, `НомерТелефонаБезКодов`, and `Значение`. The provider now fails closed unless the catalog includes `Ref_Key`, `Description`, `НаименованиеПолное`, `ВидКонтрагента`, `Покупатель`, `Поставщик`, `Недействителен`, `DeletionMark`, `IsFolder`, those two lookup fields, the contact collection, and `Комментарий`. The Admin audit also performs a bounded `$top=1` read-only lookup-projection probe and reports only `PASS`, `NO_SAMPLE`, or `FAIL`; it never returns contact values. The current synced `one_c_counterparties` projection is not used for authoritative matching because production evidence shows its phone/e-mail coverage is currently zero.

### Provider-neutral request and data minimization

The worker DTO contains only `customerIdentityId`, `provisioningJobId`, `sourceOrderId`, stable `operationKey`, `customerKind`, display name, verified phone, and optional normalized e-mail. IDs support Portal correlation and are not business identity. OTPs, Auth/session metadata, payment/MAIB payloads, browsing data, devices, notification state, Agent attribution and security internals never enter 1C.
...
- Phone is candidate evidence only. Candidate discovery uses the bounded phone/e-mail search fields, while the decision reads all values from the authoritative `КонтактнаяИнформация` collection. A candidate is sufficient only when one normalized verified phone matches and either one exact normalized e-mail or exact whitespace/case-normalized display name also matches.
...
### Create blocker, read-back and unknown-result safety

Production metadata proves the embedded contact-row shape, but it does not prove which governed `Catalog_ВидыКонтактнойИнформации.Ref_Key` values must be used for customer phone/e-mail or whether this configuration requires an additional `Catalog_ФизическиеЛица` write/link during counterparty creation. Generic OData collection POST availability is not sufficient evidence of the accounting-domain create contract. Consequently the production provider contains no executable create POST and fails closed with `ONE_C_CUSTOMER_CREATE_CONTRACT_NOT_PROVEN`; `ONE_C_CUSTOMER_WRITE_READY` cannot override this safeguard. The exact remaining contract is the Finance/1C-approved create payload, required contact-kind references, and post-create linkage/read-back invariants.

The implemented idempotency seam remains ready for that proven adapter: PostgreSQL can durably set `create_attempted_at`, the future payload must carry `NOVOTECH_FINAL_CUSTOMER:<operation_key>` in `Комментарий`, and a successful response will never be sufficient by itself. Read-back uses `Catalog_Контрагенты` by `Ref_Key` and validates active person type plus authoritative contact/name evidence before the atomic completion RPC.

If a future proven POST times out, the response is lost, or the process crashes after the marker is stored, every subsequent claim enters reconciliation-only mode. It searches the stable marker with bounded `$top=5`; exactly one result is read back, multiple results become `AMBIGUOUS`, and zero results remain retryable. The worker never issues a second create for that job.
...
Read-only production data establishes `Catalog_Контрагенты` as the current customer master and confirms active `ФизическоеЛицо` rows. The live audit returned HTTP 200 and proved that the earlier scalar phone/e-mail assumption was invalid; the authoritative metadata snapshot proved the two search properties and embedded contact-row collection described above. Runtime processing remains disabled until the corrected contract is deployed and accepted. Creation remains separately blocked on the exact 1C contact-kind/create contract and explicit Owner approval; no production 1C customer was created.
